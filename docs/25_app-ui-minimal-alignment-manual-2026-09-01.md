...
日期：2026-09-01
状态：**Step 0–10 全部已落地（Step 0–2 于 2026-09-01，Step 3 于 2026-09-02 rebase 合入，Step 4–5 于 2026-09-02 在 main 工作树实施并已 commit，Step 6–7 于 2026-09-03、Step 8–9 于 2026-09-04 同样在 main 工作树实施并已 commit；Step 10 同日实施并已 commit）**。本文是可执行手册，不是设计随笔；每个 Step 都能独立开工、独立验证、独立回滚。
...
10. **抽屉**：~~Mode 是 Light/Dark/System **三选**（Minimal 的二态开关无法表达 `system`，而它正是 `defaultMode`）~~ **2026-09-06 推翻并删除**——`system` 整个从产品移除（默认改为亮色，`defaultMode="light"`，`public/theme-init.js` 归一化并写回旧存值），抽屉不再有 Mode 块，light/dark 唯一入口是 header 的 `ThemeModeButton`（72d8a4b 引入）；连带删除 `useSettingsReset`——摘掉 mode 折叠后它只剩对 context 的纯转发，且留着会让红点亮起时抽屉里无对应项、「全部重置」还会偷改配色。Presets 用 Minimal 的 sidebar 字形色板而非手册措辞里的「圆点」——顺带让颜色演示它真正要染的东西，六色各有 i18n 名（两个蓝，序号不可读）；标题叫「外观 / Appearance」而非「设置」（设置页已占这个词）；~~`useSettingsReset()` 把「mode ≠ system」折进 `canReset`，抽屉 Reset 与 header dot 共用一个答案~~（同日删除，红点改为直接读 context 的 `canReset`）。
11. **图标**：新增 5 个离线图标 `solar:sun-bold-duotone`/`solar:moon-bold-duotone`/`solar:monitor-bold-duotone`/`mdi:contrast-circle`/`mdi:arrow-collapse-horizontal`（Mode 用单色 duotone：`OptionButton` 选中要把字形染 `primary.main`，多色 `custom:*` 不吃 `currentColor`）。**2026-09-06 前三个随 Mode 块删除**，净剩两个。`custom:sun-color`/`custom:moon-color` 保留给 welcome 药丸。
...
> **2026-09-03 已落地**（`82c6d80`）。实际与手册的偏差记录在本节末"执行记录"。
...
> **2026-09-04 已落地**（`f11b8ed`）。实际与手册的偏差记录在本节末"执行记录"。
...
> **2026-09-04 已落地**（`5374d87`）。实际与手册的偏差记录在本节末“执行记录”。
...
- ~~改：`.trellis/spec/frontend/directory-structure.md`~~ —— **该文件不存在，此条无对象**，改为核对根 `CLAUDE.md` 目录索引 + `entrypoints/app/CLAUDE.md` 并在 `index.md` 如实标注状态（勘误 D-5）
...
grep -rn "MUI v7\|Chrome 116\|segmented\|header-actions" CLAUDE.md entrypoints .trellis/spec   # 零结果 → 见勘误 D-6：判据改为人工判读，命中里不得有「当前有效的描述性事实」
```

**回滚点**：`docs(app-ui): sync specs, CLAUDE.md and ui baseline after Minimal alignment`。

**完成判据**
- [x] 附录 C 零 `[UNKNOWN]`（2026-09-04 复核，见附录 C 表末的 Step 10 复核段）
- [x] `ui-design-system.md` 无 docs/23 时代残留值（§4 primary 双 scheme 常量、§6「8px 贯穿」+「dark card: none」、§7 输入框 48/40 与 Button 三档、§11 Menu list padding 五处全部改掉；§8 shell 变量逐个对过源码，准确，未改）
- [ ] 截图目录存在且 README 列出核对结论 —— **本轮挂起，留给人工**：Chrome 未带 `--remote-debugging-port` 启动（`DevToolsActivePort` 是 09-01 的陈旧文件，9222 无监听），`docs/ui-baseline/app-runtime-check.mjs` 无法附着，脚本按设计直接失败而不是静默降级。按 spec §16「Hand-off before finish-work」交接：build 已是最后一次源码编辑之后的产物，路径 `C:\Users\18368\Desktop\00_myCode\24_cyberSquirrel\00_favbase\.output\chrome-mv3`；重跑方式 = 带 `--remote-debugging-port=9222` 启动 Chrome、加载该 unpacked 目录、开一个 `app.html` 标签，再 `node docs/ui-baseline/app-runtime-check.mjs docs/ui-baseline/2026-09-04/minimal-alignment`

#### Step 10 执行记录（2026-09-04，main 工作树）

1. **本步唯一的代码改动是一条断言**，其余全是文档/规范同步：`entrypoints/app/theme/theme-contract.test.ts` 新增 `it.each(PRESETS)` × `colorKeys.palette`，锁 `Label variant="inverted"` 的 `palette[color].darker` on `palette[color].lighter` ≥ 4.5。复用该文件现成的 `PRESETS`/`themeFor`/`paletteOf`/`contrastRatio`，色名从 `core/palette.ts` 的 `colorKeys.palette` 取（不手写字符串数组：手写会在加色时静默漏掉）。实测 primary default 8.44 / preset1 11.49 / preset2 11.90 / preset3 11.84 / **preset4 7.95** / preset5 9.01；secondary 11.19 / info 10.81 / **success 6.89** / warning 7.43 / error 9.00 —— 全局最低 6.89，无预设需要回退。**两 scheme 只跑一轮**：dark 把这一对色前后景互换，对比度比值对称；且只有 primary 随预设变，另五色是固定 ramp，逐预设重跑等于免费复验。**跨 Step 遗留复核项由此清零**（用户 2026-09-04 决定保留变体、不删）。该断言做过变异检验（把门限临时抬到 7 → 六个预设各在 success 上报 `expected 6.885153588318012 to be greater than or equal to 7`，失败信息带预设与色名），不是只会绿的空断言（spec §16：Any audit that can only pass is not an audit）。
2. **竖向 Tabs 不统一**（用户 2026-09-04 决定，Step 7 推来的一致性复核结论）。spec §11 新增 "Vertical Tabs" 小节，锁死两种形态：(a) 二级导航吃主题默认下划线（唯一先例 `sections/settings/section-rail.tsx`），(b) 卡内数据行兼段选择器隐指示条 + `tabsClasses.list` 4px 行距 + 44 高 + 0.75 半径 + 8% 品牌洗底（唯一先例 `sections/overview/analytics-platform-composition.tsx`）；按「行是导航还是数据」选，**禁止统一**，§15 加一行禁令，§10 与 §11 Settings 两处各加一句反向指路。理由写进 spec：下划线加在环图图例旁标不出弧和色点没标过的东西且与洗底同行打架；把 8% 洗底搬到 settings rail 会与 `components/nav-section/` 的激活治法（同洗底 + 同 accent 墨）重影，把页内二级导航拉平成一级导航。三处目录 `CLAUDE.md`（overview / settings / theme）同步指向该规则。
3. **不新建 `directory-structure.md`**（勘误 D-5，用户 2026-09-04 决定）。`.trellis/spec/frontend/` 实际只有三个文件，`index.md` 表里 14 行有 12 行悬空（只有 `i18n-conventions.md` 与 `ui-design-system.md` 落地）；本文铁律 5/8 引用的两个正在悬空之列。改为把前六行如实标 `Not created` + 指出事实的真实 owner，后六个领域 spec **在本步复核时一并标 `File missing` 并逐行点名 owner**（初稿只把它们留在 `Active`，与表下的审计段自相矛盾——状态列描述的是这份 guide，文件不在盘上就不是 `Active`；`[UNKNOWN]` 只剩「是否曾写过」这一层，`.trellis/` 未进 git），并在表下写一段 2026-09-04 链接审计。不新建的理由与 Step 5 不新建第二个守卫测试同理：同一条规则两处执行，必有一处腐掉。
4. **`grep` 零结果判据改人工判读**（勘误 D-6）。实测 11 处命中，`MUI v7`/`Chrome 116` 零命中，其余全是禁令正文、已删文件/死 CSS 的出处说明、以及 `indicatorColor="custom"` 的形态命名——删掉反而丢信息。判据改为「命中里不得出现当前有效的描述性事实」。
5. **spec 的失真点是逐条对着 `theme/core/**` 源码校准的，源码一行未改**（反过来就是把收口做成了改设计）。具体：§4 的 `primary.lighter #FEE9E1 (light) / #3A2A24 (dark)` 已失真（Step 2 起是一条 ramp 两 scheme 共用），补 `contrastText` 与「预设整条换 + `contrastText`/`text.accent` 派生」；§6 把 cards/menus/popovers/dialogs 全归 8px 是错的，改成 base 8 向下 0.5/0.75/1 + 主题向上派生表（Card·Dialog·Skeleton 16 / Popover·Menu 10 / Avatar 12 / Tooltip·MenuItem 6 / Chip 字面量 8·10），并删掉「dark Cards 用 hairline + `card: 'none'`」改成两 scheme 都投真实阴影（dark 走 black channel）+ 提两个 CSS 变量 hook；§7 的输入框 48/40 是 docs/23 时代值（实为 56/40 且主题不设 `minHeight`，D11）、Button 只写三档（实为 30/36/48/56 且默认 `color="inherit"`）、`MuiPaper` 写成根上就有 quiet outlined border（实为仅 `outlined` 变体有边 + `elevation: 0`）、Tabs/Tab 补四个 defaultProps 并说明 48px 是沿用 MUI 默认；§11 的「Menu: 4px list padding」说反了（4px 是 Popover **paper** 的 padding，list 上下为 0，C-4）。
6. **§8 Shell 复核后未改**：九个 `--layout-*` 与六个 `--nav-*` 逐个对过 `layouts/core/css-vars.ts`、`layouts/dashboard/css-vars.ts`、`components/nav-section/styles/css-vars.ts`，全部准确（Step 4 已在当步同步过 spec）。
7. **头部的 `DESIGN.md` 悬空引用**（D-1）在 spec 头部与 `entrypoints/app/index.html` 的 FINISH 段两处都改指 docs/25，并显式写明「根目录没有 `DESIGN.md`」，避免下一个人再去找。`index.html` 的 OWN-WORLD 段同时从「8px base radius … low-strength light card shadow」换成 Minimal v7.7.0 视觉语言 + 品牌 token 的实况。
8. **`entrypoints/app/CLAUDE.md`** 首段仍写 docs/19 的「目录卡片库 / material-kit-react 方向」，是当前有效文档里的失真，改成 Minimal v7.7.0 全套 + 品牌 token 并点明 §3.1 推翻、§3.2 保留；§约定 段删掉「值已从 material-kit-react 换成目录卡片库方向」「Phase 1」等措辞，补上向上派生的三档半径归主题。
9. **根 `CLAUDE.md`** 顺手修掉 Step 9 漏收的一处：`lib/chat` 索引行把 `history.ts` 写成 WXT storage，实为 PGlite `chat_conversations`。docs/25 索引行状态改 Step 0–10 全部已落地，并补 Step 10 摘要与「遗留项已消解」的实测值。
10. **截图基线本轮挂起**（用户 2026-09-04 决定），原因与交接方式见上面第三条完成判据。不做像素 diff（本文 §5 Step 10 原就要求人工核对清单）。
11. **验证**：`pnpm vitest run entrypoints/app/theme/theme-contract.test.ts entrypoints/app/components/label` 2 文件 / 45 例绿 → `pnpm compile` 零错 → `pnpm test` 六次里三次红（1–3 例，全是 `lib/database/proxy-db.test.ts` 与 `lib/bilibili/transcribe-utils.test.ts` 的异步顺序断言，隔离重跑全绿，且本步未碰 `lib/**`；已知 CPU 争用抖动），空跑一次 194 文件 / 1,413 例 + 工作区包 10 文件 / 55 例全绿 → `pnpm build` 绿（background 契约 11 模块 / 939,265 B）→ `pnpm zip` 绿（`.output/favbase-0.0.5-chrome.zip`，17,563,798 B，只验打包不上传；复核追加 `index.html` 注释后重打为 17,563,875 B，+77 B 全在 `app.html` 的注释里）→ `git diff --check` 干净。体积测法与前九行一致（`gzip -c`，默认压缩级）。
12. **已提交** `5374d87`（`.trellis/spec/frontend/index.md` 的 Git 政策是 AI 默认不 commit；本步是用户在同一对话里明确要求提交）。改动 10 个文件：`CLAUDE.md`、`docs/23`、`docs/25`（本文）、`entrypoints/app/index.html`、`entrypoints/app/CLAUDE.md`、`entrypoints/app/theme/CLAUDE.md`、`entrypoints/app/theme/theme-contract.test.ts`、`entrypoints/app/components/label/CLAUDE.md`、`entrypoints/app/sections/overview/CLAUDE.md`、`entrypoints/app/sections/settings/CLAUDE.md`，外加未进 git 的 `.trellis/spec/frontend/{ui-design-system,index}.md`。
13. **复核（同日 check pass）又抓出六处失真，逐条改在文档侧、源码仍是零改动**：(a) spec §4「Dark mode」的 `Text primary #F4F6F8` / `Text secondary #C4CDD5` 是 docs/23 时代值，主题从来是 Minimal 暗色墨（白 / grey 500 / grey 600，D12），改正并注明旧值出处；(b) §4「app canvas is exact」漏了唯一会搬动它的设置——high-contrast 把 **light** 底换成 `grey.200` 且 `card` 换 `z1`（`with-settings/update-core.ts`），补一段；(c) `index.md` 的六个领域 spec 初稿留在 `Active` 而文件不存在，与同一页的审计段自相矛盾，改 `File missing` + 逐行点名 owner + 去掉点不开的链接（D-5 回写）；(d) D-5 与本记录第 3 条把 `index.md` 的表写成「12 行链接 10 行悬空」，实为 **14 行 12 行悬空**；(e) 根 `CLAUDE.md`「参考项目」段仍把 `06_material-kit-react` 写成"app.html 布局骨架与主题机制参考 + 视觉 token 已按 docs/19 换成目录卡片库方向"——按 D-6 判据这是"当前有效的描述性事实"却已被 Step 1 推翻，改成 Minimal v7.7.0 源码根为当前唯一 UI 参考、material-kit 降为历史。(f) §7 开头「every one of them is locked in `theme/theme-contract.test.ts`」是假的——该测试根本没读 `MuiTabs`/`MuiTab`、`MuiAvatar`、`MuiTableCell`（Tabs 的 defaultProps 只被 `settings-navigation.test.tsx` 的渲染断言间接碰到）；这种无条件的「全部锁住」最危险，读者会把没人守的值当有人守。按「文档与源码不符就改文档」改成列出实锁清单 + 点名三个未锁族（补断言留给后续 Step，本步不扩范围）。另 `index.html` 的 `FORM:` 行（catalog-card library / seed key）保留但标明是生成谱系而非现行方向。**仍待人工**：同注释 `FIRST VIEWPORT` 行的「40px status row / 40px filter row」是 docs/19 时代的实测值，本轮截图基线挂起故无法复测，`[UNKNOWN]`——恢复远程调试端口后与截图基线一并核。
14. **复核的验证跑法与结论**（同日）：`pnpm vitest run entrypoints/app/theme/theme-contract.test.ts entrypoints/app/components/label` 2 文件 / 45 例绿；新断言做了独立变异检验（门限 4.5 → 7，六个预设各在 `success` 上报 `expected 6.885153588318012 to be greater than or equal to 7`，改回后复跑绿），六色 × 六预设的对比度由 check pass 用独立脚本重算一遍，与记录逐位一致（primary 8.44 / 11.49 / 11.90 / 11.84 / **7.95** / 9.01；secondary 11.19 / info 10.81 / **success 6.89** / warning 7.43 / error 9.00），且 `components/label/styles.tsx` 的 dark 分支确认互换的就是同一对 `lighter`/`darker`（不是换成别的阶），故只读 `light` scheme 的论证成立；`pnpm compile` 零错；`pnpm test` 两次各红 1 例（先 `lib/database/db.test.ts` 后 `lib/database/proxy-db.test.ts`，均为 5 秒 `testTimeout` 超时，隔离重跑分别 1.49 s / 1.51 s 全绿，两次红的还不是同一个文件——CPU 争用的典型形状，且本步零 `lib/**` 改动），`pnpm -r test` 包 10 文件 / 55 例绿；`pnpm build` 绿且产物与 HEAD 构建逐字节相同；`git diff --check` 干净。
...
| 10 | `index.html` 契约注释、`ui-design-system.md`（头部 + §4/§6/§7/§10/§11/§15/§16；§8 复核后无需改）、~~`directory-structure.md`~~ → **`.trellis/spec/frontend/index.md`**（该文件不存在，六个悬空链接如实标注，D-5）、`docs/23` 注记、根 `CLAUDE.md`（docs/25 索引状态 + Step 9 漏收的 `lib/chat` `history.ts` 一处）、**`entrypoints/app/CLAUDE.md`**（首段与 §约定 仍写 docs/19 的目录卡片库方向，失真）、**`entrypoints/app/theme/CLAUDE.md`**（Tabs 竖向两形态指路 + 测试清单补 `inverted` 断言）、**`components/label/CLAUDE.md`**（遗留复核项已消解）、**`sections/{overview,settings}/CLAUDE.md`**（各自认领一种竖向 Tabs 形态并写「禁止统一」）——首轮清单只列了前五项，见 Step 10 执行记录 |
...
| 9 | 已落地 2026-09-04，已 commit（待六项目测） | `9424ac0` | app 91,616 B（+53）；Container（共享 MUI）122,157 B（+478）；jsx-runtime 56,544 B（−5）；chat 路由 chunk 54,155 B（**app 与 chat 两个数不可复现**：Step 10 的 check pass 把工作树 stash 回本提交重建，得 91,617 / 54,162，Container 与 jsx-runtime 分毫不差；即这两个数量自本提交之前的一次构建，Step 10 未回改本行以保留原始记录） | `chat-view.tsx` 656 → 190 行编排 + 八个子文件；**用户决定**助手回答不进气泡（320 宽放不下 markdown）；`styles.tsx` 不建、`chat-nav-drawer.tsx` 顶替；三处手册勘误（layout 无 CSS 变量 / 无重命名 API / 文件名 compose）；详见 Step 9 执行记录 |
| 10 | 已落地 2026-09-04，已 commit（截图基线本轮挂起待人工） | `5374d87` | app 91,617 B；Container（共享 MUI）122,157 B；jsx-runtime 56,544 B；chat 54,162 B（**四个 chunk 与 HEAD=`9424ac0` 的构建逐字节相同**：check pass 把工作树 `git stash` 到 Step 9 的提交点重跑 `pnpm build`，app/chat/Container 三个 chunk 的 md5 与 gzip 字节完全一致，所以 Step 10 对产物零影响、`index.html` 的注释只进 `app.html` 不进任何 JS chunk。与 Step 9 行记录的 91,616 / 54,155 的 1 B 与 7 B 差**不是测量噪声**——gzip 是确定的，同一提交点重测就是 91,617 / 54,162；那两个数是 Step 9 在最后一次改动之前的构建上量的（chat chunk 内容变一点 → 它的 hash 变 → app chunk 里那条 import 说明符跟着变，正好解释 1 B/7 B 的组合）。测法：`gzip -c <chunk>`（默认级；注意 `gzip -c file` 会把文件名写进 header，`cat file | gzip -c` 少 16 B，行间要可比就必须统一这一种）） | 唯一代码改动是 `theme-contract.test.ts` 的 `inverted` 对比度断言（跨 Step 遗留项清零，最低 6.89:1）；spec §4/§6/§7/§11/§16 按源码校准 + 新增竖向 Tabs 两形态规则 + §15 一行禁令；`index.md` 六个悬空链接如实标注；三条用户决定（Tabs 不统一 / `inverted` 保留 + 断言 / 不新建 `directory-structure.md`）+ 两条新勘误（D-5、D-6）；`pnpm test` 六次里三次因 CPU 争用红（`lib/**` 异步顺序断言，隔离全绿），空跑一次 194 文件 / 1,413 例 + 包 10 文件 / 55 例全绿；`pnpm zip` 出 `favbase-0.0.5-chrome.zip` 17,563,798 B（不上传）——复核追加了 `index.html` 注释后重打为 **17,563,875 B**，+77 B 全在 `app.html` 那条注释上，JS chunk 逐字节未变；详见 Step 10 执行记录 |
...
**Step 10 复核（2026-09-04）**：C-1 至 C-7 七条全部是"已消解 + 实际结论"的结论式表述，无一条留悬念，本表零待办。正文里剩下的 `[UNKNOWN]` 字样只有两类，均非未决分支：(1) Step 1 改法段的 `[UNKNOWN C-2]`/`[UNKNOWN C-3]` 是**当时**指向本表的施工指针，其结论已写在同 Step 的执行记录里（"`[UNKNOWN]` 消解"两段），保留以便读者对照施工前后；(2) 铁律 10 与 Step 10 判据里对本表的引用。唯一新增的 `[UNKNOWN]` 在 D-5，且是被显式记录的"不猜"（`.trellis/` 未进 git，无法判断六个领域 spec 是否曾存在）。铁律 10 达成。
...
- **D-5**（Step 10 发现）Step 10 那条"改 `.trellis/spec/frontend/directory-structure.md` 补新目录"**无对象**：该文件不存在。`.trellis/spec/frontend/` 实际只有三个文件（`index.md` / `i18n-conventions.md` / `ui-design-system.md`），`index.md` 的 Guidelines Index 里有 14 行，其中 12 行链接是悬空的（只有 `i18n-conventions.md` 与 `ui-design-system.md` 有文件）——六行标着 "To fill"/"Active" 但从未建过（`directory-structure` / `component-guidelines` / `hook-guidelines` / `state-management` / `quality-guidelines` / `type-safety`），另六行标 "Active" 的领域规范（database-bridge…extension-host-permissions）同样无文件。本文 §4 铁律 5 引用的正是 `directory-structure.md`、铁律 8 引用的正是 `quality-guidelines.md`，两条铁律的出处一直是空链接。处置（用户 2026-09-04 决定）：**不新建**——目录归属的事实已住在根 `CLAUDE.md` 目录文档索引 + `entrypoints/app/CLAUDE.md`，验证顺序已住在本文 §6 与 spec §16，同一条规则放两处必有一处腐掉（与 Step 5 不新建第二个守卫测试同理）；改为在 `index.md` 把前六行如实标 `Not created` 并指出事实的真实 owner，后六行标 `File missing` + 逐行点名 owner（**初稿把它们留在 `Active`，本步复核时改掉**：文件不在盘上就不是 `Active`，那与表下审计段自相矛盾；不猜的只是"是否曾写过"——`.trellis/` 未进 git，无法判断是从未写过还是随 worktree 拆除丢失，这一层标 `[UNKNOWN]`）。十二行一律去掉 markdown 链接：留着点不开的链接就是留着一个假承诺。铁律 5/8 的正文语义不变，只是出处改指真实 owner。
- **D-6**（Step 10 发现）Step 10 验证命令里 `grep -rn "MUI v7\|Chrome 116\|segmented\|header-actions" CLAUDE.md entrypoints .trellis/spec  # 零结果` **不现实，且强行清零会丢信息**。2026-09-04 实测 11 处命中（`MUI v7` 与 `Chrome 116` 零命中），分三类，全部应当保留：(a) **禁令正文**——spec §11 的 "no segmented pill skin"；(b) **已删文件/死 CSS 的出处说明**——`theme/mode-transition.ts` 与 `layouts/components/language-popover.tsx` 各自记录"从 `header-actions.tsx` 搬出"，`layouts/CLAUDE.md` 与本文 Step 4 摘要记录该文件被删，`entrypoints/app/CLAUDE.md` 用 `segmentedTabsSx` 当 v9 死类名的活例子，`settings-navigation.test.tsx` 注释说明 112/116px 下限随 segmented 皮肤一起删了所以轨道该横滚；(c) **形态命名**——`core/components/tabs.tsx` 的 `indicatorColor="custom"` 就是分段（药丸）形态，`theme/CLAUDE.md` 与 spec §7 如实描述它（app 无消费者）。判据改为**人工判读**：命中里不得出现"当前有效的描述性事实"（例如"侧栏用 segmented Tabs"、"最低 Chrome 116"这类把已推翻结论写成现状的句子）；历史出处、禁令与形态命名不算违规。
