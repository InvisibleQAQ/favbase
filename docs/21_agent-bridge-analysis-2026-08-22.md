...
- **DB 单持有者 = Offscreen**；app.html 经 Background SW PortBridge 三跳 RPC（`lib/background/port-bridge.ts:1-8`）。生产 SW 尚未持有 db proxy，但 Phase 0 探针已在 SW 内实测 `ensureOffscreen()` → `initDbProxy(ensureOffscreen)` → `hybridRetrieve` 成功。PortBridge 对无 `sender.url` 的 SW 自连会忽略（`port-bridge.ts:36`），offscreen 的 `DatabaseRpcHandler` 直接接收同名 `chrome.runtime.connect`（`rpc-handler.ts:56`），所以无需新增中继特殊情况。
...
- **Manifest**：权限 `storage/unlimitedStorage/alarms/offscreen/declarativeNetRequest/cookies/webRequest/bookmarks/favicon`（`wxt.config.ts:62-78`）；无 `nativeMessaging`、无 `externally_connectable`、**无 `minimum_chrome_version`**；CSP `script-src 'self' 'wasm-unsafe-eval'; object-src 'self'`，无 `connect-src` 限制（`wxt.config.ts:90-93`）；`http://localhost/*` host pattern 已因 Ollama 存在（`lib/providers.ts:184`）。
...
| 存储 | `lib/storage/keys.ts` 新增 `local:agent-bridge`（`{ enabled, port, token, tokenCreatedAt }`）与 `local:agent-bridge-status`（`{ state, lastConnectedAt, lastError }`） | 经 `lib/storage` facade；`defaults` 集中 | 端口默认值经 `lib/env.ts` `envNumber('VITE_AGENT_BRIDGE_PORT', <默认>)`，守卫测试要求 `.env.example` 同步 |
...
| DB | `entrypoints/background.ts` | `ensureOffscreen()` 后 `initDbProxy()`（惰性，首个 tool 调用时） | Phase 0 已实测通过 |
...
| Manifest | `wxt.config.ts` | `minimum_chrome_version: '116'`；`wxt.config.test.ts` 断言 | 无新权限（`alarms` 已有；Windows Chrome 149 实测 loopback WS 不需 host permission） |
