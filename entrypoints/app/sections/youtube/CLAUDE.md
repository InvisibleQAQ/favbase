...
- `youtube-view.tsx` — scaffold Adapter；常驻 pipeline 为 Fetch → Embedding/Tagging 并行，运行时显示抓取计数/可用百分比，完成后保留本次数量 + 100%；三条 lane 统一读 `backgroundJobRuntime` 的 phase 和单按钮控制。未配置/auth gate 隐藏 strip；现有 phase、设置跳转、错误翻译与标签职责不变。
