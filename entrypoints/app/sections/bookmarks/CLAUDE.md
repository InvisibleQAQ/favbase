...
- `bookmarks-view.tsx` — scaffold Adapter：常驻 pipeline 为 Sync → Extraction → Embedding/Tagging 并行；后两条 lane 来自 `trackJobRun`，coverage 来自 DB。文件夹、卡片、page-scope operation 与 `showSyncButton=false` 语义不变。
...
- `use-bookmark-extraction.ts` — 正文提取是 `bookmarks:sync` job；逐条 Embedding/Tagging 仍立即 fire-and-forget，并经 `trackJobRun` 聚合为独立 lane 供 pipeline 观察。完整 progress、pause 边界、跨路由与 pending 续传语义不变。
...
- 标签/Embedding：成功提取后仍逐条立即启动，避免 run 末批处理制造 orphan；独立 `bookmarks:embed|tag` job 仅观察重叠 Promise，不接管、排队或延迟工作。
