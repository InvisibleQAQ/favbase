...
- `use-bookmarks.ts` — 数据 hook。挂载时 `sync()` 自动跑一次，负责本地书签树同步、分页查询与元信息刷新；**同步成功后链式 `startBookmarkExtraction()`**（恢复 `5a04c87` 删掉的链路，控制面改由闸门承担）。搜索 300ms 防抖，folderId/search/page 驱动查询，undefined folderId = 「全部」。
