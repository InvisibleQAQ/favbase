...
- `use-bili-fav-folders.ts` — B站收藏夹 hook：mount 只 `fetchAndSyncFolders()`，不扫 historical pending。手动 Fetch 把路由选中 Source 移到 folders 首位后调用 `runBiliStreamingSync`；优先级属于 Fetch producer，Transcript 不维护第二套排序。`bilibili:sync` job 跨 remount 去重并注入 checkpoint
...
- `auto-transcribe-bar.tsx` — 纯进度展示。`configuration_required` 持久 warning 链到 `/settings?section=asr`，同 runtime 保存有效 ASR 后自动恢复；`quota_paused` 显示 durable guard。无阶段开关或历史 pending preview
