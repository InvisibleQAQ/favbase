...
- `auto-transcribe-bar.tsx` — 纯进度展示。`configuration_required` 不再自绘 warning，而由页面统一 `CollectionConfigurationNotice` 消费该权威 phase；同 runtime 保存有效 ASR 后仍由 watcher 自动恢复。`quota_paused` 继续在本组件显示 durable guard。
