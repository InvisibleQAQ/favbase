...
- `bilibili-view.tsx` — 页面 Adapter：通过共享 pipeline 展示 Fetch/Transcription/Embedding/Tagging；Fetch 完成保留本次总数 + 100%，四阶段 runtime 统一读 background job phase 并只显示一个暂停/继续控制。认证门隐藏 strip，禁止用 `indexing` 冒充 Tagging。标题、服务端搜索、换夹 remount 与 operation scope 不变。
...
- `use-bili-fav-folders.ts` — B站收藏夹 hook：挂载时只 await `fetchAndSyncFolders()` 并发布 folders，避免路由进入触发全量视频分页；显式同步按钮通过模块级 `bilibili:sync` job 跑 `syncAllFavoriteVideos()`，跨 remount 去重并把 checkpoint 传入分页 worker。auth + API fetch + DB sync 全部由 service 内聚
