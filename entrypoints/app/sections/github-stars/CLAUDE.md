...
- `language-chips.tsx` — 语言 chip 行：共享 `ChipRowShell`（github icon + `githubStars.languagesTitle`，header icon 继承 shared secondary 色）+ `FilterChip`（maxWidth 220），本文件保留内容逻辑——「全部(N)」chip + 各语言(count)（服务层已按数量降序），chip icon 为数据色点 `LanguageDot`。库空时由 view 隐藏整行
