...
import { colorKeys } from '../../theme/core/palette';

const baseColors = ['default'] as const;
const allColors = [...baseColors, ...colorKeys.palette, ...colorKeys.common] as const;

/**
 * Status pill shared by every page. Four variants over the palette + common
 * colors, all driven by the `softStyles` / `filledStyles` mixins so a palette
 * swap (Step 2 color presets) carries through without touching this file.
 */
export const LabelRoot = styled('span', {
  ...
  variants: [
     ...
     * @variant inverted
     *
     * Deliberate reversal: ink text on a tinted wash in light, the mirror in
     * dark. The `lighter` / `darker` steps here are the point of the variant —
     * they are NOT the "light wash used as a selected background" that docs/25
     * Step 2 replaces with `varAlpha`.
     */
    {
