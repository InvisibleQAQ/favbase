...
/**
 * The sonner skin (Minimal `components/snackbar/styles.tsx`), with two
 * deliberate favbase deviations recorded in docs/25 Step 5:
 *
 * - width 360 instead of Minimal's 300, and the toast surface is
 *   `theme.mixins.paperStyles` rather than a flat `background.paper`, so a
 *   toast reads like every other floating surface in app.html (menus, popovers).
 * - the severity icon wash stays Minimal's single `currentColor` rule rather
 *   than four per-severity `varAlpha(<color>.mainChannel, 0.08)` rules. Same
 *   pixels, one rule instead of four — `varAlpha` resolves `currentColor`
 *   through `color-mix()`.
 */
...
const toastStyles = (theme: Theme): CSSObject => ({
  [`& .${snackbarClasses.toast}`]: {
    ...
    // Type-less `toast('…')`: inverted ink surface, so it must drop the paper
    // wash as well as the paper colour.
