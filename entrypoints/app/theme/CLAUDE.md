...
- `theme-provider.tsx` — ThemeVarsProvider + CssBaseline 包装。`defaultMode="system"`（首次跟随 OS）+ `modeStorageKey=COLOR_MODE_STORAGE_KEY`（`'favbase-color-mode'`，localStorage 持久化）。导出 `COLOR_MODE_STORAGE_KEY` 常量，**必须与 `entrypoints/app/index.html` 的 FOUC 内联脚本 key 保持一致**
...
- **FOUC 防闪**：MUI 的 `InitColorSchemeScript` 是 React 组件，CSR（Vite/WXT）下客户端注入的 `<script>` **不会执行**，故改用 `index.html` `<head>` 手写内联脚本：读 `localStorage['favbase-color-mode']`（默认 system → `matchMedia` 解析）→ 首帧设 `data-color-scheme`。改 key/attribute/默认模式时三处同步：index.html 脚本、theme-provider、theme-config
