...
/**
 * Menu item styles shared by MenuItem, Autocomplete options and nav dropdowns.
 *
 * @example
...
/**
 * Translucent, blurred paper with two faint corner washes. `dropdown` adds the
 * popover padding, shadow and 10px (`8 × 1.25`) radius.
 *
 * @example
...
// Radial washes encoded once (https://www.fffuel.co/eeencode/); CSS background
// data: URIs are not governed by the extension's script-src CSP.
const cyanShape =
...
/**
 * Filled / soft color variants for Button, Chip, Fab, Pagination, Badge, Avatar.
 *
 * @example
...
export function softStyles(theme: Theme, colorKey: ColorKey, options?: StyleOptions): CSSObject {
  ...
  // favbase override: coral `dark` is 3.99:1 on the 16% wash; `text.accent` is
  // the scheme-aware brand text shade (light darker 8.5:1 / dark light 6.2:1)
  // and already flips per scheme, so primary skips the `applyStyles('dark')`.
  const softText: CSSObject =
    colorKey === 'primary'
