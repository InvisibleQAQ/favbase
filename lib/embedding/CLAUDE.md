...
- `indexing.ts` — `indexItemChunks(db, itemId, chunks, deps?) → 'chunked' | 'embedded'`：**内容类型无关**编排（零平台知识）。流程：`replaceItemChunks` 落库 → content_state='chunked' → `resolveEmbeddingConfig().enabled` 才 embed（`createEmbeddingModel` + `embedTexts` → `upsertChunkEmbeddings` → 'embedded'）。embed 任何失败（含 EmbeddingDimensionError/网络/未配置）catch + console.error，停在 'chunked' **不向上抛**；chunk 落库失败则向上抛（调用方决定）。`IndexingDeps`（getConfig/embed）可注入测试（镜像 PipelineDeps DI 风格）
- `config.ts` — `resolveEmbeddingConfig(settings)` 纯函数（镜像 `resolveAsrConfig`）：`UserSettings` → `{ providerId, apiKey, baseUrl, model, enabled }`。**逐字段优先级：`.env.local`（`VITE_EMBEDDING_*`，非空即赢）> 用户填写 `embeddingConfigs[providerId]` > provider def**。`VITE_EMBEDDING_PROVIDER` 须为合法 `EmbeddingProviderId`（按 `EMBEDDING_PROVIDER_IDS` 校验，非法值忽略沿用后备链）；baseUrl/model 的 def 后备来自 resolved provider。`VITE_EMBEDDING_ENABLE` 是硬覆盖（大小写不敏感 `true`/`1` 启用、`false`/`0` 禁用，未设置/不可识别才回退 `settings.embeddingEnabled ?? false`）。env 是"默认凭证包"，与激活 provider 无关（`import.meta.env` 构建期内联，改 `.env.local` 需重跑 build）。`getEmbeddingSettings()` 异步便利（getValue + resolve，非 React 消费者用）
