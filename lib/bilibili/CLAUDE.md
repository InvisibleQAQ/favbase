...
- `bili-sync-service.ts` — Bilibili 领域同步深 Module。收藏浏览与入库分离：`fetchFavoriteVideosPage()` 仅返回 UI 请求页；自动转录专用 `fetchAndSyncVideos()` 必须 await 当前页持久化，Source 缺失或写入 dropped 时直接失败；`syncAllFavoriteVideos(onProgress?, control?)` 把最小 cooperative checkpoint 传给 runner，在下一收藏夹/分页前暂停。`listFoldersWithPending(mediaIds)` 零网络单查询（sources ⋈ item_sources ⋈ items）返回**保持输入顺序**的、含 ≥1 条 `contentState='pending'` 视频的夹 id 子集——自动转录接续的派发过滤器（守护测试 `list-folders-with-pending.test.ts`，含 platform-scoping）。内容路径以 `persistContentChunks(bvid, rows, source) → 'chunked'|null` 暴露 durable-chunks seam；旧 `persistContent()` 保留为兼容组合入口
