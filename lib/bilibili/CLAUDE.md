...
- `bili-sync-service.ts` — Bilibili 领域同步深 Module。收藏浏览与入库分离：`fetchFavoriteVideosPage()` 仅返回 UI 请求页；自动转录专用 `fetchAndSyncVideos()` 必须 await 当前页持久化，Source 缺失或写入 dropped 时直接失败；`syncAllFavoriteVideos()` 组装 API/DB/延迟依赖后委托 runner 完成全部收藏夹同步。内容路径以 `persistContentChunks(bvid, rows, source) → 'chunked'|null` 暴露 durable-chunks seam；旧 `persistContent()` 保留为兼容组合入口
...
- `transcribe-utils.ts` — `transcribeAndPersist`：durable chunks 成功后发 `item-content-updated`，并行启动 Embedding/Tagging；仍只 await Embedding。可选 `onEmbeddingRun/onTaggingRun` 只暴露原 Promise 生命周期供 app job store 观察，不改变等待与失败语义。
- `transcription-coordinator.ts` — 手动转录纯 JS 协调器。构造器可注入 transcription run 与独立 `embed|tag` run observer；coordinator 不 import app store，无参构造仍兼容。缓存预载、stale generation、取消与 `indexing` UI 契约不变。
- `auto-transcribe-adapter.ts` — B站自动转录 Adapter；可注入 `trackProcessingRun` 将每条 Embedding/Tagging Promise 上报 app 层，认证、分页、pending、失效项和转录 Interface 不变。
