...
- **content_state='chunked' + 延迟 embed（D3）**：description 即内容，非空 → 全文写 `item_contents` + `charSplit(preferParagraph:true)` 切块 → `'chunked'`，**不 inline embed**；空 description → `'no_content'`（**不用 `'pending'`**——那会喂给 auto-transcribe）。向量化推迟到设置页「重建向量」。同步后 ILIKE 即可搜；语义检索需先重建
