...
- **content_state='chunked' + 延迟 embed（D3）**：description 即内容，非空 → 全文写 `item_contents` + `charSplit(preferParagraph:true)` 切块 → chunk 行写成后才置 `'chunked'`（事务内中间态 `'has_content'` + 幽灵同步自愈——全量重拉手握 description 顺手补写，见 `lib/ingest/CLAUDE.md`），**不 inline embed**——同步收尾 embed lane 排空平台积压（**ST3 触发在 hook 层经 `startJob`**，非本 wrapper），设置页「重建向量」为手动兜底；空 description → `'no_content'`（**不用 `'pending'`**——那会喂给 auto-transcribe）。同步后 ILIKE 即可搜；语义检索在自动 embed 完成后可用
