...
- **content_state='chunked' + 延迟 embed（D3）**：description 即内容，非空 → 全文写 `item_contents` + `charSplit(preferParagraph:true)` 切块 → `'chunked'`，**不 inline embed**——同步收尾 `embedNewItems` 自动向量化新条目，设置页「重建向量」为积压兜底；空 description → `'no_content'`（**不用 `'pending'`**——那会喂给 auto-transcribe）。同步后 ILIKE 即可搜；语义检索在自动 embed 完成后可用
