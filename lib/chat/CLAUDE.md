...
Chat（Agentic RAG 知识库助手）平台无关 lib 层，供 app.html `sections/chat` 消费。只读 PGlite。

## 当前状态（P1 + P2 + P3 + P4）

`config.ts`（LLM 解析）+ `retrieval.ts` / `rrf.ts` / `types.ts`（Hybrid 检索核心）+ `tools.ts` / `prompts.ts` / `agent.ts`（agent 循环 + 工具）+ `history.ts`（多会话持久化）已落地。剩 P5 收尾。

## 只读铁律

本 lib 全程只读 PGlite——只发 `SELECT` / `db.execute(sql\`SELECT ...\`)`，绝不 insert/update/delete/DDL。检索工具层自我限制。
...
- `tools.ts` — AI SDK v6 `tool()` 定义，纯对象 registry `chatTools = { searchKnowledgeBase, getItemContent, listTags }`（**key 即模型看到的 tool 名**，prompt/描述用同名交叉引用）。DB handle 经 `streamText` 的 `experimental_context` 注入，tool `execute(input, { experimental_context })` 里 `contextDb(...)` 取 `db`（缺则抛，`ChatToolContext` 契约共享给 `agent.ts`）。**全部只读**（SELECT）。inputSchema 遵循 course 规则：`.describe()` 放 `z.object(...)` 链尾、`platform` 用 `z.enum(COLLECTION_PLATFORMS)`、字段 snake_case、返回把关键字段拍平到顶层。
  ...
  - `listTags`：包 `getAllUsedTags(platform?)`。字段 `platform`?(enum)。返回 `{ count, tags: [{ id, name, count }] }`。
...
- `history.ts` — 会话 CRUD over WXT `local:` storage（`storage.defineItem<ChatConversation[]>`，key `STORAGE_KEYS.chatConversations` = `local:chat-conversations`，单数组存全部会话）。**只写 WXT storage，绝不写 PGlite**（只读知识库铁律）。
  - `ChatConversation = { id: string; title: string; modelMessages: ModelMessage[]; createdAt: number; updatedAt: number }`。**持久 `modelMessages`（模型态，含 tool-call/tool-result 轮次）为唯一事实源**——display 气泡 + 每条 assistant 的来源卡片都在加载时从它重建（见 `sections/chat/use-chat-agent.rebuildDisplayMessages`）。`id` 用 `crypto.randomUUID()`（app.html 可用）。
  - CRUD：`listConversations()`（`updatedAt` 降序）/ `loadConversation(id)` / `saveConversation(conv)`（upsert，存前过 `trimMessages`）/ `createConversation()`（新空会话，未落盘）/ `deleteConversation(id)`。
  - `deriveTitle(modelMessages)` — 从首条 user 消息取文本、折叠空白、截断 40 字（`modelMessageText` 兼容 string / text-part 数组两种 content）。
  - `trimMessages(messages, max=MAX_MESSAGES=40)` — 滑动窗口：取末 `max` 条后**丢弃开头非 user 消息**，使窗口从 user 轮开始，避免持久态里孤立的 tool-result 脱离其 tool-call（部分 provider 会拒绝）。
...
- `history.test.ts` — mock `wxt/utils/storage`（同 `lib/storage/x-last-sync.test.ts` 内存 stub）：CRUD（空态 / save+load / upsert 不重复 / 降序 list / delete）+ `deriveTitle`（首 user 消息 / 折叠空白 / 截断 / 无 user→'' / text-part 数组）+ `trimMessages`（cap 内不动 / 超 cap 取末段且首条为 user / 丢弃开头孤立 tool-result）。
