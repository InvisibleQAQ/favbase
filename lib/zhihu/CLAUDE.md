...
- `zhihu-sync-service.ts` — **DB schema 知识的唯一持有者**（`PLATFORM='zhihu'`）。同步：`syncFavorites(onProgress?)`（fetchAllFavorites → `syncFavoritesToDb` → `void tagNewItems('zhihu', newItemIds)` 自动打标 + `void embedNewItems('zhihu', newItemIds)` 自动 embed，均 fire-and-forget——触发在本 wrapper 而非 ToDb：仅 app.html context 可读 LLM/embedding 配置，且守护测试走 ToDb 不触外部 API；docs/16 MEDIUM-2 同款接缝）、`syncFavoritesToDb(db, collections, favorites)`（导出供测试：先跨夹去重 + turndown 转 Markdown（CPU 活不进事务）→ 事务骨架委托 `ingestCollection`（`lib/ingest/`，docs/16 HIGH-1）——本函数只声明归一化行：sources=**全部公开收藏夹含空夹**（title 重命名经 upsert 刷新）、items 含 platformMeta/contentState、links 按收藏 membership、content=`{textOf: markdownById, chunk: charSplit(·, {preferParagraph:true})}`）。查询（UI 零 drizzle 导入）：`getFavorites({collectionId?, search?, page, pageSize})`（`publishedAt DESC NULLS LAST`；collectionId 走 `EXISTS(item_sources⋈sources)` 子查询；search 对 title/authorName/platformMeta excerpt ILIKE，转义 `%_\`）、`getCollectionCounts()`（有条目的收藏夹按计数降序，chip 行——空夹自然不出现）、`getLastSyncedAt()`（`max(lastFetchedAt)`）。UI 行类型 `ZhihuFavoriteItem` 在此导出。守护测试 `zhihu-sync-service.test.ts`（in-memory PGlite）
...
- **content_state**：answer/article/pin（有正文）→ Markdown 落 `item_contents.plainText` + `charSplit(preferParagraph:true)` 切块 → `'chunked'`（**不 inline embed**，D3——同步收尾 `embedNewItems` 自动向量化新条目，设置页「重建向量」为积压兜底）；zvideo/空正文 → `'no_content'`（**不用 `'pending'`**——那会喂给 auto-transcribe）
