...
- `zhihu-sync-service.ts` — **DB schema 知识的唯一持有者**（`PLATFORM='zhihu'`）。同步：`syncFavorites(onProgress?)`（fetchAllFavorites → `syncFavoritesToDb`）、`syncFavoritesToDb(db, collections, favorites)`（导出供测试：事务前先跨夹去重 + turndown 转 Markdown（CPU 活不进事务）→ 单事务 sources upsert（**全部公开收藏夹含空夹**，title 重命名+lastFetchedAt 经此刷新）→ authors/items/item_sources insert-only → 事务外对**新插入 item** 写 content+chunks）。查询（UI 零 drizzle 导入）：`getFavorites({collectionId?, search?, page, pageSize})`（`publishedAt DESC NULLS LAST`；collectionId 走 `EXISTS(item_sources⋈sources)` 子查询；search 对 title/authorName/platformMeta excerpt ILIKE，转义 `%_\`）、`getCollectionCounts()`（有条目的收藏夹按计数降序，chip 行——空夹自然不出现）、`getLastSyncedAt()`（`max(lastFetchedAt)`）。UI 行类型 `ZhihuFavoriteItem` 在此导出。守护测试 `zhihu-sync-service.test.ts`（in-memory PGlite）
