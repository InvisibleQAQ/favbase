...
- `main.ts` — 薄路由（~45 行）：模块加载时 `ffmpeg.start()` + `db.start()`，单一 `onMessage` 分发 4 种消息（STATUS 组合两侧 getState() / CHUNK_RELEASE / CHUNK_PREPARE / CHUNK_TRANSCRIBE），无业务逻辑。**07-20：X 书签同步 runner（`x-sync.ts`）+ `OFFSCREEN_X_SYNC` 路由已删——x.com 浮层按钮整体移除，X 同步统一走 app.html（页面 context 直接 `syncBookmarks`，经 RPC proxy 写本 offscreen 的 PGlite）**
