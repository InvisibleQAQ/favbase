...
- `x-sync.ts` — X 书签同步 runner（服务 x.com `/i/bookmarks` 浮层按钮，CS 无法直连 PGlite/读认证）：`runXSync(sessionId, auth)` 先 `await initDbMain()`（幂等，等 PGlite ready，否则 `getDb()` 抛）再跑 `lib/x` 的 `syncBookmarks(auth, onProgress→OFFSCREEN_X_SYNC_PROGRESS)`；**永不 reject**，返回判别式 `{success:true,result} | {success:false,error:XSyncError}`（`classifyXSyncError` 分类）。**offscreen 文档没有 `chrome.storage`（只有 runtime 消息 API）**，不能自己 `getXAuth`——auth 由 background SW 解析后随 `OFFSCREEN_X_SYNC` 消息传入（同 `apiKey` 之于 CHUNK_TRANSCRIBE 的模式）；扩展上下文 fetch 回放捕获 header（无 DNR、无 `credentials:'omit'`，见 `lib/x/CLAUDE.md`）。**本路径不自动打标**（LLM 配置同样读不到；docs/16 MEDIUM-2 显式欠账，决策见 `lib/x/CLAUDE.md`）——`SyncBookmarksResult.newItemIds` 随 done 消息透传但无消费方
- `main.ts` — 薄路由（~50 行）：模块加载时 `ffmpeg.start()` + `db.start()`，单一 `onMessage` 分发 5 种消息（STATUS 组合两侧 getState() / CHUNK_RELEASE / CHUNK_PREPARE / CHUNK_TRANSCRIBE / **X_SYNC → `runXSync`**），无业务逻辑
