...
- `types.ts` — `AutoTranscribeAdapter`、`AutoTranscribeVideo`、phase/state/stats/quota 类型。`configuration_required` 与 `quota_paused` 都是保留当前 session 的可恢复等待态；`asrBlocked` 独立表示是否存在停放的缺 ASR 配置条目。
- `pipeline.ts` — `AutoTranscribePipeline.createSession()` 创建单 producer inbox。session `append()` 按规范化 video id 去重并增长真实 total，`run(control?)` 串行领取条目，`close()` 表示 producer 结束；只有 close、队列排空且停放的 ASR 条目已恢复后才完成。普通单条失败 mark error 后继续，Fetch 生命周期完全不可见。

## 约定

- session 输入必须是已经持久化的新 Collection Item 事实；禁止在本模块重建 Source/folder/page crawler 或 historical pending scan。
- 缺 ASR 只在转录返回 `ASR_INVALID_KEY` 且当前配置确实无 key 时触发。当前条目进入 session 内的 ASR parked 列表，后续视频继续消费；单个 `waitForAsrKey()` watcher 恢复后，停放条目按原顺序重新入队，经 cooperative checkpoint 后重试。没有后续可运行条目时 phase 才是 `configuration_required`，`asrBlocked` 是独立的 UI 阻塞信号。
