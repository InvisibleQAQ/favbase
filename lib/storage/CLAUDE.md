...
- `keys.ts` — STORAGE_KEYS 静态 key 注册表 + STORAGE_PREFIXES 动态 key 前缀注册表，新增 key/前缀先在此检查冲突。含 `chatConversations: 'local:chat-conversations'`——Chat（Agentic RAG 助手）多会话历史（单数组存全部 `ChatConversation`），`storage.defineItem` 落点在 `lib/chat/history.ts`（非本目录，chat 自包含），是 chat 唯一持久化（只读知识库，绝不写 PGlite）
