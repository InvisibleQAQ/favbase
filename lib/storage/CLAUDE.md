...
- `ui-state.ts` — sidebarPinnedStorage（`local:sidebarPinned`）+ localeStorage（`local:locale`）+ `asrQuotaPauseStorage`（`local:asr-quota-pause`，nullable `{ providerId, resetAt }`，只作 ASR 自动转录重启 guard）+ **xLastSyncStorage**（`local:x-last-sync`，nullable X 最近一次成功同步摘要）
