...
- Bilibili **Sources** are public favorites folders only (user decision 2026-09-22): a private folder is not a Source and its videos are not Collection Items, even though the logged-in folder listing returns it. Zhihu already applies the same rule through `is_public`. Enforced where the folder listing is fetched: private folders are dropped before anything else sees them (`fetchFavFolders`, docs/29 Step 4).
- "Source" also appears in the subtitle vocabulary, where it says how a transcript was obtained: the platform's own subtitle ("official") or favbase's speech recognition ("asr"). That subtitle source is not a **Source**: it describes a transcript and has no relation to the containers a Collection Item was collected through. When both could be meant, say subtitle source; stored subtitle sources are kept only for transcripts, so other content has none (docs/29 Step 5).
