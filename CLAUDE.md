...
- 转录总流程: handler 解析 auth + CID → pipeline 统一编排（cache → subtitle_check 官方字幕优先（fetchOfficialSubtitle dep，含重试）→ ASR fallback（transcribeAudio dep：connectivity → extractBiliAudioUrl → fetchAudioBlob → assertAudioNotReused → ≤24MB 直传 / >24MB Offscreen FFmpeg 分块）→ deps.postProcess（平台特有字幕后处理，B 站注入 processSubtitles）→ cache save）。TranscribeRequest.cid 可选：content script 有 cid 就传，app.html 不传由 handler 解析。ASR 配置（apiKey/model/baseUrl）统一由 `AsrConfig` 类型承载，`resolveAsrConfig` 是唯一真实来源
