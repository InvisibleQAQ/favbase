...
- `lib/transcription/pipeline.ts` — TranscriptionPipeline 深模块：`runTranscriptionPipeline(request, deps, onProgress)` 编排完整转录策略（cache → official subtitle → ASR fallback → process → cache save）。`PipelineDeps` 5 方法接口（`getAsrConfig`/`fetchOfficialSubtitle`/`transcribeAudio`/`cacheGet`/`cacheSave`），转录策略的唯一真实来源。`processSubtitles` 直接 import（纯函数，不通过 DI）。`AsrConfig`（apiKey/model/baseUrl）是 ASR 配置统一类型。`toErrorInfo()` 错误映射内聚于此
...
- `lib/background/transcription-handlers.ts` — handleTranscribe 薄 dispatcher（auth + CID 解析 → 构造 5 个 PipelineDeps → runTranscriptionPipeline → 通知 tab）。`createFetchOfficialSubtitle(auth)` 内聚重试逻辑（最多 2 次，1s/2s 间隔），`createTranscribeAudio(tabId, ctx)` 内聚 connectivity + download + fingerprint + direct/chunked + offscreen 会话管理。+ handleTranscribeAbort + handleOffscreenProgress
...
- 无字幕降级: useSubtitle 返回 no_subtitle（网络异常也降级为 no_subtitle 而非 error）→ App 显示 TranscribeButton（status 为 no_subtitle 或 error 时均展示）→ 用户点击 → TRANSCRIBE_AUDIO → Background handler（auth + CID 解析 → pipeline 统一编排）→ 结果回写 SubtitleView
...
- 转录总流程: handler 解析 auth + CID → pipeline 统一编排（cache → subtitle_check 官方字幕优先（fetchOfficialSubtitle dep，含重试）→ ASR fallback（transcribeAudio dep：connectivity → extractBiliAudioUrl → fetchAudioBlob → assertAudioNotReused → ≤24MB 直传 / >24MB Offscreen FFmpeg 分块）→ processSubtitles（pipeline 直接 import）→ cache save）。TranscribeRequest.cid 可选：content script 有 cid 就传，app.html 不传由 handler 解析。ASR 配置（apiKey/model/baseUrl）统一由 `AsrConfig` 类型承载，`resolveAsrConfig` 是唯一真实来源
