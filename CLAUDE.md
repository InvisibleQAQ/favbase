...
- `lib/background/transcription-handlers.ts` — 有状态 handler 统一签名 `(msg, sender, ctx)`。`handleTranscribe`：dedup 检查（`ctx.startTranscription` 返回 null → 返回 TRANSCRIBE_DUPLICATE 错误）→ auth + CID 解析 → 构造 5 个 PipelineDeps → `runTranscriptionPipeline` → `ctx.finishTranscription` 清理。`createFetchOfficialSubtitle(auth)` 内聚重试逻辑（最多 2 次，1s/2s 间隔），`createTranscribeAudio(tabId, ctx)` 内聚 connectivity + download + fingerprint + direct/chunked + offscreen 会话管理（通过 `ctx.registerChunkSession`/`unregisterChunkSession` 路由进度）。`handleTranscribeAbort` 委托 `ctx.abortTranscription`。`handleOffscreenProgress` 通过 `ctx.resolveProgressTarget` + `ctx.getActiveTranscriptions` fallback。零模块级状态
