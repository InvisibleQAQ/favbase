...
- `.trellis/spec/frontend/platform-onboarding.md` — **接入新收藏平台的可执行契约**（2026-09-06，docs/26 Step 3 全面改写于 2026-09-07）：先做的五个决策 → `lib/<platform>/` 领域层（共享 ingest/查询骨架，可在翻转判别符前独立建成并测绿）→ 翻转 `COLLECTION_PLATFORMS` 让 `tsc` 与契约测试生成 TODO 清单 → **§6 两份 Platform Descriptor（领域五字段 §6.1 + UI 五字段 §6.2，ADR 0004）+ §6.3 四处重值注册表** → `sections/<platform>/` → 凭据链 → **§9 无守卫人工清单只剩两条**（凭据链无交叉校验、英文硬编码文案；两条的共性是**都不是「平台的事实」**，那就是 descriptor 的能力边界，原七条的生死在 docs/26 附录 A）。**§2 的守卫表是本文最重要的一节**：五个自动守卫，其中三个「会红但活儿仍是你手写的」——`.env.example` 平台块、welcome marquee 药丸（D5 刻意不派生）、`skills/favbase/SKILL.md` 的**两份**平台清单（`<platform>` 句 + frontmatter `description`）。`tests/platform-completeness-contract.test.ts` 是它的可执行半边
