...
- `.trellis/spec/frontend/platform-onboarding.md` — **接入新收藏平台的可执行契约**（2026-09-06）：先做的五个决策 → `lib/<platform>/` 领域层（共享 ingest/查询骨架，可在翻转判别符前独立建成并测绿）→ 翻转 `COLLECTION_PLATFORMS` 让 `tsc` 与契约测试生成 TODO 清单 → 13 张契约表 + 3 张仅类型表 → `sections/<platform>/` → 凭据链 → **六条无守卫覆盖的人工清单**（后台任务指示器 `PLATFORM_LABEL`、`SOURCE_DIMENSION` 的 `Partial`、`lib/chat/tools.ts` 的三处 prompt 字面量、凭据链无交叉校验、英文硬编码文案、`.env.local` 文档块）。`tests/platform-completeness-contract.test.ts` 是它的可执行半边
