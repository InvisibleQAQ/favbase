...
- `lib/storage.ts` — UserSettings 类型定义 + settingsStorage (WXT `storage.defineItem<UserSettings>`)，DEFAULT_SETTINGS 默认值，sidebarPinnedStorage（`local:sidebarPinned`，布尔值，默认 true）。resolveAsrConfig(settings)（纯函数，UI 和 pipeline 共享的 ASR 配置解析逻辑，从 `asrConfigs[asrProvider]` 读取 apiKey/model + 从 `getAsrProviderDef` 读取 baseUrl，返回 `{ apiKey, model, baseUrl }`，是 ASR 配置的唯一真实来源）。migrateSettingsIfNeeded()（一次性迁移：旧平铺 ASR 字段 → `asrConfigs` Record）
...
- `lib/bilibili/videos-sync.ts` — syncFavVideosToDb(db, videos, sourceId)：将 BiliFavVideo[] 批量 upsert 到 PGlite authors + items + item_sources 表，返回 `SyncResult`（total/synced/dropped/droppedBvids）。事务包裹（原子性），3 张表各一次 INSERT ON CONFLICT DO NOTHING + 一次 SELECT 获取 ID 映射（~5 RPC 替代原 N*3 串行）。映射失败时 console.warn 记录具体 bvid + 失败原因（author mapping miss / item mapping miss），不静默丢弃。事务整体失败返回 SyncResult（synced:0）而非 throw。输入 author 按 platformAuthorId 去重。MVP insert-only 不更新已有记录。由 `useBiliFavVideos` fetch 成功后 fire-and-forget 调用
- `lib/bilibili/content-sync.ts` — persistSubtitleContent(db, bvid, rows, source)：转录成功后将字幕文本持久化到 PGlite item_contents 表 + 更新 items.content_state='has_content'。按 bvid 查找 item，onConflictDoUpdate upsert。fire-and-forget 调用，不抛异常。由 `transcribeAndPersist()` 成功回调触发
- `lib/bilibili/transcribe-utils.ts` — 转录共享工具函数（`TranscriptionCoordinator` 和 `AutoTranscribePipeline` 共用）：`transcribeAndPersist(bvid, title)` 发送 TRANSCRIBE_AUDIO + 成功后 fire-and-forget persistSubtitleContent；`createStatusListener(matchBvid, onProgress)` 安装 TRANSCRIBE_STATUS 监听器，返回 cleanup 函数
...
- `lib/bilibili/auto-transcribe-pipeline.ts` — 自动转录编排深模块（纯逻辑，零 React 依赖）。`AutoTranscribePipeline` class：`subscribe()`/`getSnapshot()` 支持 `useSyncExternalStore` 订阅，`start(mediaId)`/`stop()` 控制接口，`queryPreview(mediaId)` idle 态预览查询。内部编排：分页爬取 → syncFavVideosToDb → 查询 pending → 逐个调用共享 `transcribeAndPersist()`（CC 5-10s / ASR 10-15s 随机间隔防检测）→ 429 暂停 60s 重试 → 失败标记 error → 下一页。`waitWithCountdown()` 封装等待+倒计时显示。通过 `transcribe-utils.ts` 共享 TRANSCRIBE_STATUS 监听
...
- `lib/transcription/types.ts` — 转录核心类型：SubtitleRow, SubtitleResult, TranscribeRequest/Abort, TranscribeResponse(success|failure), TranscribeStage(12种 union), TranscribeStatusPush(stage enum + stageParams), TranscribeErrorCode(14种) + TranscribeErrorInfo(code + debug message + params), PipelineError(域内错误基类，AsrError/AudioExtractError/AudioReuseError 继承)
...
- `lib/transcription/pipeline.ts` — TranscriptionPipeline 深模块：`runTranscriptionPipeline(request, deps, onProgress)` 纯函数编排转录 8 阶段（cache check → settings → connectivity → extract → download → fingerprint → transcribe/chunk → process+cache）。不依赖 Chrome API，通过 `PipelineDeps` 接口注入所有外部依赖。`AsrConfig`（apiKey/model/baseUrl）是 ASR 配置统一类型，`PipelineDeps` 的 `ensureConnectivity`/`transcribeDirect`/`transcribeChunked` 均通过参数接收 `baseUrl`（不依赖闭包）。`toErrorInfo()` 错误映射 + `transcribeWithFakeProgress()` 进度模拟内聚于此
- `lib/offscreen/types.ts` — Offscreen 协议类型：ChunkPlan, OffscreenPrepareRequest/TranscribeRequest/ReleaseRequest, OffscreenProgressMessage/ResultMessage/ErrorMessage, OffscreenRequest/OffscreenMessage union
...
- `lib/offscreen/main.ts` — Offscreen Document 逻辑：fetchAudioBytes(自行 fetch audioUrl) + resolveAudioDuration(HTML5 Audio 优先，ffprobe 降级) + FFmpeg 本地 WASM 加载(public/ffmpeg/) + estimateSafeChunkSeconds(0.72 安全系数) + buildOverlappedChunkPlan(600s+4s overlap) + splitAudioIntoChunks(FFmpeg -c:a copy) + transcribeChunk(委托 `requestGroqTranscription`，鸭子类型检查 `.info` 属性提取 `TranscribeErrorInfo` 保持 IPC 序列化兼容，零运行时类依赖) + mergeTimestampedChunkRows(时间偏移+overlap 裁剪+1.5s 近邻去重)。最多 3 轮 30% 缩减。Session Map 带 10min TTL 自动清理（60s 扫描）+ FFmpeg 操作失败后 resetFFmpeg() 防止状态污染
...
- `entrypoints/background.ts` — Background SW thin dispatcher：initPortBridge(同步注册 onConnect) + BackgroundContext + `onMessage` switch 路由到独立 handler + onInstalled/onStartup 确保 Offscreen 存活
...
- `lib/background/transcription-handlers.ts` — handleTranscribe（cache check → CID 解析（caller 提供或 fetchCidByPageList 降级）→ 官方字幕优先（fetchSubtitle + processSubtitles + mergeVideoCache）→ ASR pipeline 降级（PipelineDeps 适配器懒读取 resolveAsrConfig → runTranscriptionPipeline））+ handleTranscribeAbort + handleOffscreenProgress
...
- B 站视频持久化: `useBiliFavVideos` fetch 成功后 fire-and-forget 调用 `syncFavVideosToDb`（返回 `SyncResult`），先查 sources 表获取 sourceId（收藏夹未 sync 则跳过）。批量 upsert + 事务保证原子性（~5 RPC 替代原 N*3 串行），映射失败时 console.warn 记录具体 bvid，事务整体失败返回 SyncResult（synced:0）不影响 UI。MVP insert-only 不更新已有记录，`content_state='pending'`
...
- 转录总流程（handleTranscribe 编排）: cache check → CID 解析（caller 提供或 fetchCidByPageList）→ fetchSubtitle 官方字幕优先（有字幕 → processSubtitles + mergeVideoCache + 返回 success）→ 无字幕 → ASR pipeline（PipelineDeps.getAsrConfig 懒读取 resolveAsrConfig → ensureGroqConnectivity → extractBiliAudioUrl → fetchAudioBlob → assertAudioNotReused → ≤24MB: requestGroqTranscription / >24MB: Offscreen FFmpeg 分块 → processSubtitles → cache）。TranscribeRequest.cid 可选：content script 有 cid 就传，app.html 不传由 handler 解析。ASR 配置（apiKey/model/baseUrl）统一由 `AsrConfig` 类型承载，`resolveAsrConfig` 是唯一真实来源。PipelineDeps 适配器懒读取 settings（不在入口闭包固化），baseUrl 通过函数参数显式传递（不捕获闭包）
- 视频缓存: `lib/cache/video-cache.ts` 模块。per-bvid 独立 key `local:vc:{bvid}`（chrome.storage 中为 `vc:{bvid}`，key 前缀由内部 `CHROME_KEY_PREFIX` 常量管理）。内存缓存层（Map<string, VideoCacheEntry>）+ Promise-based per-bvid 写锁 + 内部 hash 去重（computeRowsHash 不导出）。调用方只传 `mergeVideoCache(bvid, rows, source)`，hash/timestamp 由 cache 内部管理。所有来源（bilibili/groq）字幕统一缓存。旧 `local:videoCache` 单体 key 在首次访问时自动迁移。Background 侧 initCacheStorageListener 保持内存缓存与 storage 同步，Content Script 侧通过 `onVideoCacheChange(bvid, cb)` 订阅跨 tab 缓存变更（key 格式封装在缓存模块内部，调用方无需知道）
