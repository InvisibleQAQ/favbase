...
- **Toast**: `sonner` 2，**只允许 `entrypoints/app/components/snackbar/**` import**（守卫 `tests/ui-vendor-boundaries.test.ts`）；业务代码用该目录导出的 `toast`。toast 只报**一次性动作结果**，持续状态（已保存徽标、连接状态 Alert、拉取进度）不进 toast（docs/25 D6 方案 A）
...
- `docs/25_app-ui-minimal-alignment-manual-2026-09-01.md` — app.html **全面向 Minimal v7.7.0 看齐**的分步改造手册（**Step 0–10 全部已落地**（Step 0–2 于 2026-09-01，Step 3 于 2026-09-02 rebase 合入，Step 4–5 于 2026-09-02，Step 6–7 于 2026-09-03，Step 8–10 于 2026-09-04）；Step 1 = `theme/core` 换血：Minimal palette/shadows/mixins/一文件一组件覆盖 + `text.accent`/coral/platform 品牌 token 保留，C-2/C-3/C-4 已消解——dark neutral 回退 `#222B34`、soft primary 文字 `text.accent`、Menu 继承 Popover paper；Step 2 = 六色预设 + 设置状态数据层：`lib/storage/theme-settings.ts`（`local:themeSettings`，无 `version` 字段，逐字段 canonicalize）→ `theme/with-settings/`（`primaryColorPresets` + D14 WCAG 派生 `contrastText`、`applySettingsToTheme` 换 primary/派生 `text.accent`/high-contrast grey-200 底 + z1 卡片阴影）→ `components/settings/` context（`SettingsProvider` 挂 `main.tsx`，`ThemeProvider` 可选读 leaf context，welcome 不受影响）；dark `primary.lighter` 再着墨特判删除，选中洗底统一 `varAlpha(primary.mainChannel, 0.08)`；C-5 全过线；抽屉 UI 留 Step 4；Step 3 = 六个共享原语（Label/EmptyContent/CustomBreadcrumbs/CustomPopover/Scrollbar/LoadingScreen）+ `theme/create-classes.ts`：其前置只有 Step 1，与 Step 2 是并列兄弟（只有 Step 4 同时需要两者），因此并列开发后再 rebase 到 Step 2 之上；C-6 已消解（EmptyContent 不设默认 title），并修掉 Minimal 在 MUI v9 下的移植缺陷（`slotProps.paper.ref` 不再转发，CustomPopover 箭头改经 `parentElement` 反查 paper）。**跨 Step 遗留复核项已于 Step 10 消解**：Label 的 `inverted` 变体以动态索引引用 `palette[color].darker` 文字 on `lighter` 底（dark 反向），是刻意反色，不属于 Step 2 第 9 点要替换的那五处浅底；Step 2 的 C-5 只验了 `text.accent` 派生阶、`color-presets.test.ts` 只验 `contrastText` on `main`，Step 4/6/8/9 都没引入 `inverted` 消费者（Step 8 的 zhihu 类型戳用 `soft`），该项一路顺延到 Step 10——现由 `theme/theme-contract.test.ts` 的 `it.each(PRESETS)` × `colorKeys.palette` 补断言：实测全局最低 **6.89:1**（success）、primary 最低 **7.95:1**（preset4），全部过 4.5，变体保留不删（用户 2026-09-04 决定），仍无消费者但不再是未验证项；两 scheme 只跑一轮（dark 互换同一对色，比值对称）；Step 4 = shell：`components/nav-section/`（vertical + mini + flyout，行几何/激活态/连接线 owner）+ `layouts/dashboard/{nav-vertical,nav-mobile,use-translated-nav}` + `layouts/components/` 四控件 + `components/settings/drawer/` 外观抽屉 + `theme/mode-transition.ts`；`nav-config.tsx` 改 `NavGroup[]` 两组；删 `nav.tsx`/`header-actions.tsx`/`layouts/nav-active.ts`；**手册未覆盖的三处按 Minimal 优先解决**——鱼骨线换 Minimal bullet + 竖脊（用户 2026-09-02 决定）、welcome 顶栏主题药丸迁到 `welcome/sections/top-bar-actions.tsx`（否则删 `header-actions.tsx` 会打断 welcome）、`compactLayout` 语义改「on = 收窄到 lg / off = 用页面自己的 cap」（每页都显式传 `maxWidth`，Minimal 的默认参数映射永不生效）；Step 5 = `sonner` toast：`components/snackbar/`（Minimal 皮肤 + `paperStyles`/宽 360 两处有意偏离）挂 `App.tsx`，六处一次性动作结果改 toast（五卡保存 / WebDAV 同步 / 清除远端 / 两个导出 / Agent Bridge 复制）；**手册三处偏离**——`handleSave` 不改返回 `boolean`（toast 就在 hook 内，返回值无消费者；改为 try/catch 顺手修掉未处理 rejection）、守卫并入现有 `tests/ui-vendor-boundaries.test.ts` 而不新建 `tests/snackbar-import-boundary.test.ts`、失败文案具体优先（复用 `settings.sync.err.*`/`export.*`/`settings.agentBridge.copy*`，`snackbar.*` 只放通用与兜底串，用户 2026-09-02 决定）；Step 6 = Dashboard：四张 KPI 卡（`AnalyticsWidgetSummary`，`common.white` 底 + 135deg 两个 48% 品牌色渐变，字段只来自 `CollectionAnalyticsSnapshot`，无数据 `—`、无 sparkline）+ 零依赖 `components/chart/`（`DonutChart` `stroke-dasharray` 六段 + `ChartLegendItem` 单行 phrasing content）+ 构成/细分/Top tags 三张 Card；`overview-view.tsx` 630 → 164 行编排 + 六个子组件；**手册十处偏离**——构成卡改上下堆叠（用户 2026-09-03 决定，`lg` 4/12 卡内仅 ~220px 放不下左右并排）、Card 标题取 `h2 + variant h4` 对齐 `SettingsPanel`、六个子组件而非四个、图例 12px 圆点取代 36px tile（`data-slot="share-bar"` 随之消失，`share-label` 保留）、段属性 `data-segment` 取代 `data-platform`（chart 原语不认识平台）、平台数保留 `1 / 6`、覆盖率卡带 caption 且删孤儿键 `dashboard.tagCount`、KPI 白底渐变照 Minimal、榜单双列断点 `md`→`lg`、`analytics-format.ts` 就近落 section；C-7 消解（Export 卡不在 `/`，大纲仍 `[1,2,2,3,2]`）；Step 7 = Settings 页：两级导航（`settings-tabs.tsx` / `section-rail.tsx`）删掉全部本地视觉覆盖改吃 theme 默认下划线 Tabs，`segmented-tabs-sx.ts` 删除，`settings-view.tsx` 成为全仓库首个 `SectionTitleBar links` 消费者（新增双语 `breadcrumbs.home`，用户 2026-09-03 决定，Step 8 六平台页复用）；**手册两处勘误已回写**——crumb `href` 是路由相对的 `'/'`（`RouterLink` 自己补 `#`，写 `'#/'` 点不回首页）、竖排指示条是 `currentColor` 墨色而非 `primary.main`（`indicatorColor` 默认 `'inherit'`）；**九处偏离手册**——rail 保留 `useMediaQuery` 的 horizontal/vertical 两形态（否则 `settings-navigation.test.tsx` 锁的 `scrollableX` 必红）、竖排 `Tab` 保留 `justifyContent: 'flex-start'`（每行带图标，MUI 居中会排成参差图标列）、不抄 Minimal 竖排先例的 `width: 200` + `borderRight`、rail 图标 22 → 24、`minWidth` 112/116 全删、`settings-view.test.tsx` 新增例断言 `links` 数据而非 mock 的 DOM（渲染层已锁在 `custom-breadcrumbs.test.tsx` 与 `section-title-bar.test.tsx`）、spec §11 与 `entrypoints/app/CLAUDE.md`/两个原语 CLAUDE.md 同 commit 收）；Step 8 = 六平台收藏页：新建 `entrypoints/app/hooks/use-collection-breadcrumbs.ts`（收藏路由祖先的唯一 owner，从 `collection-platform-registry.ts` 派生，`(platform | null, leaf?)`，末项永远无 href），`CollectionPageCopy.breadcrumbs?` 经 scaffold 转 `SectionTitleBar links`，七个收藏页（六平台 + 聚合页 + 两个配置门早退分支）全部接入；chip 一律吃主题默认 soft（`FilterChip` 选中仍是 filled primary，`CollapsibleChipRow` 的展开 chip 刻意留 outlined），zhihu 类型戳 Chip → `Label variant="soft"`，`CoverBadge` 圆角 0.75；**用户 2026-09-04 决定**面包屑末项取导航名并把 `collections.sidebarTitle` 中文对齐成「B 站收藏夹」；**手册四处勘误**——六个 `*-view.test.tsx` 根本不存在（覆盖改由 hook 单测 + scaffold 契约 + 两个页面测试 + 平台契约测试四层承担）、zhihu 类型戳本就没有 `data-slot`、`soft` 自 Step 1 起已是 `MuiChip.defaultProps`（真实改动是删 `outlined`）、聚合页不走 scaffold；**四处偏离**——hook 多 `leaf?` 且不 memo（`t` 稳定，memo 会冻死 locale）、bookmarks 的 `:folderId` 不加级（可选筛选 ≠ 必经层级）、配置门早退也传 `links`、多改三处同屏 chip；Step 9 = Chat shell：`chat-view.tsx` 656 → 190 行编排 + 八个子文件（`layout`/`chat-nav`/`chat-nav-drawer`/`chat-nav-item`/`chat-header`/`chat-message-list`/`chat-message-item`/`chat-message-input`），rail 320 可折叠 96、行 72 高 + 48 首字头像、header 72 承载唯一 h1 + 当前会话名、composer 换 `InputBase`（`minHeight: 56`）；**用户 2026-09-04 决定**用户提问吃 Minimal 气泡（`maxWidth 320` + 16% 主色）而**助手回答保持满 760 轨道无框**（320 放不下 markdown 表格/代码块，且 `chat-markdown` 的代码块本就画在 `background.neutral` 上）；**手册三处勘误**——Minimal `layout.tsx` 没有 `--nav-width` 变量（是 `chat-nav.tsx` 的模块常量）、runtime 无重命名 API 故 header 不放 `CustomPopover` 菜单、`chat-header-compose.tsx` 更名 `chat-header.tsx` 且 `styles.tsx` 不建（两个 styled 都无对应物）；**六处偏离**——composer 是 `minHeight` 不是 `height`、发送/停止按钮留在行内、断点仍 `lg`、激活行用 8% 主色而非 `action.selected`、子组件各自 `useTranslation()` 不透传 `t`、spec §11/§12 同 commit 改；Step 10 = 收口（**唯一代码改动是那条 `inverted` 对比度断言**，其余全是文档/规范同步）：`index.html` 契约注释换 Minimal v7.7.0 视觉语言 + 悬空 `DESIGN.md` 引用改指 docs/25；spec §4（primary 一条 ramp 两 scheme 共用 + 预设整条换 + `contrastText`/`text.accent` 派生）·§6（base 8 向下 0.5/0.75/1、向上归主题 Card·Dialog·Skeleton 16 / Popover·Menu 10 / Avatar 12 / Tooltip·MenuItem 6 / Chip 字面量 8·10，两 scheme 都投真实阴影）·§7（Button 四档 30/36/48/56、输入框 56/40 且不设 `minHeight`、Paper 仅 outlined 有边、Tabs 沿用 MUI 48px）·§11（4px 是 Popover **paper** 的 padding，list 上下为 0）逐条校准，§8 shell 变量复核后确认准确无需改；**竖向 Tabs 两形态定案**（用户 2026-09-04 决定，Step 7 推来的一致性复核）——二级导航吃默认下划线（settings `SectionRail`）/ 卡内数据行兼段选择器隐指示条 + 8% 洗底（Dashboard 图例），**禁止统一**，进 spec §11 + §15；`.trellis/spec/frontend/index.md` 的六个悬空链接如实标注 `Not created` 并指向真实 owner；新增勘误 D-5（`directory-structure.md` 不存在，不新建）·D-6（那条 `grep` 零结果判据不现实，改人工判读）；截图基线本轮挂起（Chrome 未带 `--remote-debugging-port`，待人工复核））：Step 0 MUI 9 + Chrome 117 → Step 1 theme/core 换血 → Step 2 六色预设 + 设置状态 → Step 3 共享原语 → Step 4 nav-section/header/设置抽屉 → Step 5 sonner → Step 6 Dashboard KPI + 原生 SVG 图表 → Step 7 Settings → Step 8 六平台页 → Step 9 Chat shell → Step 10 收口；每步八段（目标/依赖/文件/改法/测试重写/验证/回滚/判据），决策来源逐条标"用户决定 / PRD 默认"，`[UNKNOWN]` 与 PRD 勘误集中在附录 C/D。执行任一 Step 前先读该文对应节
- `.trellis/spec/frontend/platform-onboarding.md` — **接入新收藏平台的可执行契约**（2026-09-06，docs/26 Step 3 全面改写于 2026-09-07）：先做的五个决策 → `lib/<platform>/` 领域层（共享 ingest/查询骨架，可在翻转判别符前独立建成并测绿）→ 翻转 `COLLECTION_PLATFORMS` 让 `tsc` 与契约测试生成 TODO 清单 → **§6 两份 Platform Descriptor（领域五字段 §6.1 + UI 五字段 §6.2，ADR 0004）+ §6.3 四处重值注册表** → `sections/<platform>/` → 凭据链 → **§9 无守卫人工清单只剩一条**（英文硬编码文案；2026-09-07 起凭据链改由 completeness contract 按 AST/`existsSync` 守**结构存在性**——§8 五条手写编辑里的第 1-4 条被守住（第 1 条只守 `configSavedAt` 键，zod 条目与 `UserSettings` 字段名自由不可断言），第 5 条 `probeReady` 仍零守卫，§8 逐条标注 checked/unchecked——「descriptor 装不下」不等于「守不住」，前七条的生死在 docs/26 附录 A）。**§2 的守卫表是本文最重要的一节**：五个自动守卫，其中三个「会红但活儿仍是你手写的」——`.env.example` 平台块、welcome marquee 药丸（D5 刻意不派生）、`skills/favbase/SKILL.md` 的**两份**平台清单（`<platform>` 句 + frontmatter `description`）。`tests/platform-completeness-contract.test.ts` 是它的可执行半边
- `docs/26_platform-descriptor-consolidation-manual-2026-09-06.md` — **Platform Descriptor 收敛手册**（2026-09-06，三步分提交；**Step 1-3 全部已落地 2026-09-07**）：把接入平台要手写的 16 处注册表收敛成 2 份 descriptor（`lib/collections/platform-descriptor.ts` 领域五字段 + `collection-platform-registry.ts` 的 `PLATFORM_META` 扩到 UI 五字段），并把 platform-onboarding.md §9 的无守卫清单从 **7 条压到 2 条**（复核时发现原 6 条外还有第 7 条：`capability-marquee.tsx` 的手工药丸表）。Step 1 零依赖前置**已落地**（`analytics-types.ts` 抽类型 / chat prompt 派生 / `.env.example` 转正 / marquee 覆盖断言 / 去掉 `overview-view.test.tsx` 四处硬编码 6）；**手册两处勘误已回写**——chat 平台字面量是 tools.ts **三处**（`:42` description 用显示名，手册漏记）且 `lib/chat/prompts.ts` 的 `CHAT_SYSTEM_PROMPT` 是手册范围外的第四处，**用户 2026-09-07 决定**四处全派生成 id 列表（显示名不可派生：`PLATFORM_META.title` 是 app 侧 `LocaleKeys`，`lib/` 引不到），并加 `lib/chat/tools.test.ts` 的「不得出示部分平台清单」守卫让 §9 第 3 条真死；**用户 2026-09-07 决定** `.env.example` 完整镜像 `.env.local` 结构（30 平台键 + LLM/ASR/Embedding/HTTP/Bridge 13 键，零密钥值，丢弃 `.env.local` 里零代码读取的 douyin 死块），守卫的 `.env.example` 半边改为**必须存在**；marquee 断言走 AST 复用已解析的 `PLATFORM_META`（手册说直接 import registry，改为零新增 import）→ **Step 2 已落地 2026-09-07**：`lib/collections/platform-descriptor.ts`（领域五字段 + `mapPlatforms` 唯一投影 helper + `PlatformReadiness`/`PlatformSortKey`/`PlatformDimensions` 三类型）与 `PLATFORM_META` 五字段（+ `palette`/`hint`/`childRoutes`，`'ink'` 表黑标品牌）两份 descriptor 落地，12 处消费点改派生，`theme-config.platform`（连 `BrandColoredPlatform`）与 `background-jobs-indicator.tsx` 的 `PLATFORM_LABEL` 整表删除，契约测试改为只查编译器看不见的部分，**manifest 字节相同 diff 已验证**（MV3 已装扩展的 `host_permissions` 一变就要用户重新授权）；**用户 2026-09-07 决定** analytics 三张维度表直接内联读 descriptor（不留派生中间层，三者各只有一个调用点）；新增三处黄金锁（`hostPermissions` flatMap 顺序 + `jobPlatform` 唯一性 + 八个品牌 hex）与两处类型抓不到的断言（`hint` 键命名、`backgroundJobPlatformLabel` 六平台文案），八项证伪全部先红后绿→ **Step 3 已落地 2026-09-07**：`docs/adr/0004` 写成（只含 D2/D3 两条形状决策，三条「从代码里看不出来」的约束是它存在的理由：`lib/` ↛ `entrypoints/`、`IconifyName`/`LocaleKeys` 住 app 侧、`wxt.config.ts` 在 Node 侧按相对路径加载 descriptor），spec §2/§6/§7/§9/§10/§11/§12/§13 + `.trellis/spec/frontend/index.md` + 本文件三处同步；**复核发现手册未料到的第 8 条无守卫项**——`skills/favbase/SKILL.md` 有**两份**手写平台清单，`tests/agent-bridge-cli-aliases.test.ts` 原来只守 `<platform>` 那句，frontmatter 的 `description:`（agent **据以选不选这个 skill** 的显示名清单）零守卫，**用户 2026-09-07 决定现在就补守卫**而非记为 §9 第三条（本 Step 唯一代码改动：一例双向断言 + `Zhihu collections` → `Zhihu favorites` 让断言精确而非启发式，先红后绿并已证伪）；**用户 2026-09-07 决定**「守卫会红但活儿仍手写」的三条（`.env.example` / marquee / SKILL.md）进 spec §2 守卫表（3 → 5 行）而不是留在 §9——§2 的定位是让机器生成 TODO，§9 的唯一价值是机器不会告诉你的那些。四项经决策**排除**：4 处重值注册表不动、不修 `App.tsx` 的六平台 eager 图、marquee 不派生、不做 `platforms/<id>/` 物理 co-location。执行任一 Step 前先读该文对应节 + §1 决策记录 + §3 跨 Step 铁律
- `.trellis/` — Trellis 开发工作流配置
- `tests/platform-completeness-contract.test.ts` — 单一聚合失败的跨层平台接入契约；自 docs/26 Step 2 起**只查编译器看不见的部分**（12 项纯数据进了两份穷举 descriptor，`tsc` 会在对象字面量上指名平台，AST 不再重读一遍）：两份 descriptor 的 `childRoutes` / `hostPermissions` 须为显式数组字面量、`dimensions.author`/`source` 必须属于 `ranked` 或为 `null`、四处重值注册表按 AST 对账（`COLLECTION_PAGE_LOADERS`/`CARD_ADAPTERS`/auto-sync/eligibility），且 `main.tsx` 不得出现 `collections/<platform>` 字面量；避免加载 DB 或页面 runtime；daily auto-sync registry（`entrypoints/app/collection-platform-auto-sync.ts`）须每平台显式 `runSync` 且不得手写 `jobPlatform`；`entrypoints/app/hooks/` 非测试模块不得 import `sections/`；每个平台的收藏页 view（顺着 `COLLECTION_PAGE_LOADERS` → 一行 re-export 的 page → `sections/` view）必须调用 `useCollectionBreadcrumbs`（docs/25 Step 8）；downstream eligibility registry（`lib/collections/platform-eligibility.ts`）六平台显式键（`null` 合法），且 `lib/collections/collection-processing-policy.ts` 不得含平台字面量/`platformMeta`/`->>`；**凭据链结构守卫**（2026-09-07，spec §8 / §9 原第 1 条）——`readiness: 'credentials'` 的平台（今为 github/youtube）须有 `sections/settings/<platform>-connection-card.tsx` 文件、`ConnSection` 联合成员、`connNavItems` 轨道项、`useSettings` 声明 API 面（函数声明或类型成员签名；`const` + shorthand 返回不算，故只改 `UseSettingsReturn` 成员名也会红）的 `derive<Pascal>Draft` / `save<Pascal>`、`configSavedAt` 键联合，Pascal 形从平台 id 派生；单向断言 platform ⊆ 联合（`'agent-bridge'` 合法在内且不是平台），**只证结构存在、不证接线正确**（`probeReady` 读错 key 仍然绿，§8 逐条标注 checked/unchecked）
...
- `tests/ui-vendor-boundaries.test.ts` — UI import 边界守卫，**两张表**。`VENDOR_RULES`（重量级依赖归属，docs/25 跨 Step 铁律 6）：`simplebar-react` 只允许 `entrypoints/app/components/scrollbar/**` import（含其 CSS）、`sonner`（Step 5 加入同表）只允许 `components/snackbar/**`，`entrypoints/**` 与 `lib/**` 其余位置一律禁止。`IMPORT_BOUNDARY_RULES`（跨入口模块图，2026-09-05）：`entrypoints/welcome/**` 不得 import `app/layouts/components` barrel（`self` 语义，叶文件放行）与 `app/components/settings`（`subtree` 语义）——welcome.html 刻意不挂 `SettingsProvider`；specifier 经别名（`@/`/`~/`）与相对路径双向解析，失败信息带 `file:line` + 理由。**两张表都配反向断言**（owner 目录确实用了该包 / welcome 确实按叶消费了 `layouts/components/*`），避免规则空转。**同类边界规则一律并进本文件**（手册原计划的 `tests/snackbar-import-boundary.test.ts` 不新建：两个文件执行同一条规则，就是规则在其中一个里腐掉）
...
- `wxt.config.ts` — built-in platform host permissions 由 `lib/collections/platform-descriptor.ts` 的 `hostPermissions` 声明（Node 侧按相对路径加载），`PLATFORM_HOST_PERMISSION_LIST` 按 `COLLECTION_PLATFORMS` flatMap 后 spread 进 manifest，顺序是契约；Bookmark `<all_urls>` 仍是显式平台 Adapter

## 参考项目

- `C:\Users\18368\Desktop\00_myCode\24_cyberSquirrel\02_Bilitato` — Bilitato 开源项目，B站视频 AI 助手，是视频转录功能的主要参考实现
- `C:\Users\18368\Desktop\00_myCode\35_minimal\minimal-dashboard\minimal-dashboard v7.7.0\Vite.js (JavaScript，TypeScript)\minimal-vite-ts-main\src` — **Minimal Dashboard v7.7.0，app.html 当前的唯一 UI 参考源**（docs/25 记为 `$MIN`）：`theme/core`、`components/nav-section`、共享原语、Dashboard/Settings/Chat 各 section 都是从这里移植的，只换品牌 token
- `C:\Users\18368\Desktop\00_myCode\24_cyberSquirrel\06_material-kit-react` — Material Kit React，MUI Dashboard 模板。**仅存历史**：它是 docs/19「目录卡片库」时代的骨架/主题参考，docs/25 Step 1 起 `theme/core` 已整层换成 Minimal，别再照它抄
...
- `entrypoints/app/layouts/CLAUDE.md` — 仪表盘布局系统（shell CSS 变量 owner）+ 侧栏 vertical/mini/mobile 三形态 + Header 右侧五控件 + `layouts/components/` 控件（`NavToggleButton`/`ThemeModeButton`/`LanguagePopover`/`SettingsButton`/`GithubButton`；`ThemeModeButton` 是 light↔dark 二态共享叶，`system` 只在外观抽屉）
...
- Minimal 共享原语（docs/25 Step 3 移植，Step 5 追加 snackbar；一目录一 CLAUDE.md）：
  ...
  - `entrypoints/app/components/snackbar/CLAUDE.md` — sonner toast region（docs/25 Step 5；**本目录是 `sonner` 的唯一入口**，只导出 `Snackbar`/`toast`，不导出 `Toaster`）
...
- `entrypoints/app/components/chart/CLAUDE.md` — 零依赖 SVG 图表原语（docs/25 Step 6 / D3：不引 ApexCharts/recharts；`DonutChart` 段 `data-segment` + `ChartLegendItem` 单行 phrasing content；图整块 `aria-hidden`，数字由调用方文本再打印一遍）
...
- `entrypoints/app/components/settings/CLAUDE.md` — 主题设置 context（docs/25 Step 2；Minimal `components/settings` 去掉 font/direction/nav/mode 的数据层：`SettingsProvider`（`main.tsx` 注入 `initialState`，WXT storage 镜像 + 回声去重）/ `useSettingsContext`（无 provider 抛错）/ leaf `SettingsContext` 供 `ThemeProvider` 可选读取；**docs/25 Step 4 追加 `drawer/` 外观抽屉**——360 宽右侧 Drawer，Contrast / Compact / 六色预设 / 全部重置，挂载点 `App.tsx`；**2026-09-06 起没有 Mode 块**——`system` 已从产品移除，配色只有亮/暗两态且默认亮色，唯一入口是 header 的 `ThemeModeButton`，`useSettingsReset` 随之删除，红点直接读 context 的 `canReset`）
- `entrypoints/app/components/tags/CLAUDE.md` — 平台无关标签 UI 子系统（hooks + popover + 筛选 chips + render-prop 网格）
- `entrypoints/app/hooks/CLAUDE.md` — app.html 共享 hooks（useCollectionLibrary 平台无关收藏页状态机，github/x/zhihu/youtube 数据 hook 的底座；useCollectionPipeline 六平台收藏页 pipeline 编排——coverage key + `pipeline.*` 标签 + Fetch→content?→Embedding→Tagging 段声明的唯一 owner；useCollectionBreadcrumbs 七个收藏页面包屑祖先的唯一 owner，从导航注册表派生，页面不手写 crumb）
- `entrypoints/app/utils/CLAUDE.md` — app.html 共享纯函数工具（formatDuration 时长格式化）
- `entrypoints/app/pages/CLAUDE.md` — 页面组件（lazy loaded）
- `entrypoints/app/sections/overview/CLAUDE.md` — 只读 Collection Analytics Dashboard（docs/25 Step 6 起是四张 KPI 卡 + 构成/细分/Top tags 三张 Card；KPI 字段映射唯一 owner `buildKpis`，无数据显示 `—`、无 sparkline；环图与图例在 `components/chart/`）；数据导出工具仅由设置页消费
...
- `entrypoints/app/sections/chat/CLAUDE.md` — Chat 一级页面（Agentic RAG 知识库助手，只读 PGlite）：多步 tool-calling agent + 流式回答 + hybrid 检索 + 可点来源卡片 + 工具四态 + 多会话持久化（PGlite `chat_conversations`）+ markdown 渲染；外壳自 docs/25 Step 9 起是 Minimal chat 卡片（rail 320/96 + header 72 + 消息区），文件 owner 表在该目录 CLAUDE.md
...
### 平台领域（lib）
- `lib/collections/CLAUDE.md` — 跨平台收藏只读领域层（分页查询 + Collection Analytics 快照）兼平台判别符与注册表之家（`COLLECTION_PLATFORMS` 单一事实源 + **`platform-descriptor.ts` 领域半边 descriptor**：`jobPlatform`/`readiness`/`hostPermissions`/`sortKey`/`dimensions` 五字段穷举 + `mapPlatforms` 唯一投影 helper，两条铁律「值导入只允许 `./platforms`」「不进 `index.ts` barrel」由 `platform-descriptor.test.ts` + `lib-import-smoke` 守；UI 半边在 `entrypoints/app/collection-platform-registry.ts`，分两份的三条约束见 `docs/adr/0004`）+ 下游 eligibility registry + 处理策略/覆盖率
...
- `lib/env.ts` — 平台数值策略参数 env 覆盖 helper（`envNumber('VITE_<PLATFORM>_<NAME>', default)`：有限非负数才覆盖、缺省/非法静默回退、默认值留平台常量定义原位），六平台全部数值标量常量经此可配置（变量注释文档在 tracked 的 `.env.example` 六个平台分组块，`cp .env.example .env.local` 后填真值；守卫 `tests/platform-env-constants-guard.test.ts` 禁平台目录裸数值 const/手写 `import.meta.env`，并锁 fallback=默认值 + 三方同步），无 CLAUDE.md（单文件）
