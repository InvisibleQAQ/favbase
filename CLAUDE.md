...
- `entrypoints/app/sections/collections/` — B站收藏夹页面组件（sidebar+grid 单页布局）
  ...
  - `use-video-transcribe.ts` — 手动视频转录状态管理 hook：批量 GET_VIDEO_CACHE 预查内容状态（翻页刷新）+ 发送 TRANSCRIBE_AUDIO 消息（不带 cid，handler 自行解析）+ TRANSCRIBE_STATUS 实时进度监听 + 单视频串行控制（activeBvid）+ useRetryCountdown 共享倒计时。成功后 fire-and-forget 调用 persistSubtitleContent 写入 PGlite item_contents 表。官方字幕优先 → ASR 降级策略由 Background handler 层统一管理
  - `use-auto-transcribe.ts` — 自动转录流水线 hook：爬取+转录一体化。自动分页获取视频（5-10s/页随机间隔）→ syncFavVideosToDb 入库 → 查询 `content_state='pending'` → 逐个 TRANSCRIBE_AUDIO（CC 间隔 5-10s / ASR 间隔 10-15s 随机化防检测）→ 下一页。PGlite `content_state` 驱动，天然断点续传。429 → 静默暂停 60s 后重试。无 ASR key 时跳过（保持 pending）。失败视频标记 `content_state='error'` 后跳过。与手动转录独立运行不冲突。切换收藏夹不中断（hook 在 CollectionsView 层，pipeline 在闭包内运行）。State 包含 `currentVideo`（cover/title/upper/duration）、`totalVideos`（收藏夹总视频数）、`currentIndex`（当前处理序号，1-based）供 UI 展示丰富进度。idle 态预加载 `previewVideo`（首个 pending 视频）和 `pendingCount`（通过 JOIN item_sources 过滤当前收藏夹），mediaId 变更或 phase 变化时重新查询
