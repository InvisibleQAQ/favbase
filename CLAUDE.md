...
- `docs/21_agent-bridge-analysis-2026-08-22.md` — Agent Bridge（外部 agent 经 MCP 检索 favbase）分析/决策/设计/路线，**Phase 0 Spike 已 GO，Phase 1 协议/registry 与 Phase 2 Node MCP 包已完成，Phase 3+ 未开工**：检索在 SW 内复用 `chatTools`，`packages/favbase-mcp` 只做 stdio/WS 哑管道；传输为扩展出站 WS（ADR 0002，不用 Native Messaging）；Q1–Q10 已决；术语 Agent Bridge / Knowledge Tool / Bridge Token 已入 `CONTEXT.md`
...
- `lib/agent-bridge/CLAUDE.md` — Agent Bridge 正式共享层：严格 v1 协议 leaf + 从 `chatTools` 派生的三工具 registry；Node MCP 通过相对 import 打包该 leaf
