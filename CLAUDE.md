...
- `tests/ui-vendor-boundaries.test.ts` — 重量级 UI 依赖的归属守卫（docs/25 跨 Step 铁律 6）：`simplebar-react` 只允许 `entrypoints/app/components/scrollbar/**` import（含其 CSS），`entrypoints/**` 与 `lib/**` 其余位置一律禁止；同时反向断言 owner 目录确实用了它，避免规则空转。**Step 5 已把 `sonner`（owner `components/snackbar/**`）加进同一张 `VENDOR_RULES` 表**（手册原计划的 `tests/snackbar-import-boundary.test.ts` 不新建：两个文件执行同一条规则，就是规则在其中一个里腐掉）
