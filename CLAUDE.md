...
- **本地 MCP**: `packages/favbase-mcp`（Node 20+，MCP TypeScript SDK 1.29.0 + `ws`，tsup ESM CLI）
...
- `docs/21_agent-bridge-analysis-2026-08-22.md` — Agent Bridge（外部 agent 经 MCP 检索 favbase）分析/决策/设计/路线，**Phase 0 Spike、Phase 1 协议/registry、Phase 2 Node MCP 包与 Phase 3 扩展 SW client/scheduler 已完成；Phase 4+ 未开工**：检索在 SW 内复用 `chatTools`，`packages/favbase-mcp` 只做 stdio/WS 哑管道；传输为扩展出站 WS（ADR 0002，不用 Native Messaging）；Q1–Q10 已决；术语 Agent Bridge / Knowledge Tool / Bridge Token 已入 `CONTEXT.md`
...
- `lib/agent-bridge/CLAUDE.md` — Agent Bridge 正式层：严格 v1 协议、三工具 registry、Background WebSocket client、持久认证退避与 30 秒 alarm scheduler；Node MCP 只打包协议 leaf
- `packages/favbase-mcp/CLAUDE.md` — Agent Bridge Node 包：stdio MCP、loopback `/bridge` WebSocket、Origin + Bridge Token hello 认证、有界等待、端口占用退出语义与进程级集成测试
