...
- `lib/storage.ts` — UserSettings 类型定义 + settingsStorage (WXT `storage.defineItem<UserSettings>`)，DEFAULT_SETTINGS 默认值，sidebarPinnedStorage（`local:sidebarPinned`，布尔值，默认 true）。ASR_FIELD_MAP（provider→settings 字段映射）+ resolveAsrConfig(settings)（纯函数，UI 和 pipeline 共享的 ASR 配置解析逻辑）
...
- `lib/offscreen/main.ts` — Offscreen Document 逻辑：fetchAudioBytes(自行 fetch audioUrl) + resolveAudioDuration(HTML5 Audio 优先，ffprobe 降级) + FFmpeg 本地 WASM 加载(public/ffmpeg/) + estimateSafeChunkSeconds(0.72 安全系数) + buildOverlappedChunkPlan(600s+4s overlap) + splitAudioIntoChunks(FFmpeg -c:a copy) + transcribeChunk(委托 `requestGroqTranscription`，catch `PipelineError` 提取 `.info` 保持 IPC 序列化兼容) + mergeTimestampedChunkRows(时间偏移+overlap 裁剪+1.5s 近邻去重)。最多 3 轮 30% 缩减。Session Map 带 10min TTL 自动清理（60s 扫描）+ FFmpeg 操作失败后 resetFFmpeg() 防止状态污染
...
- B 站视频持久化: `useBiliFavVideos` fetch 成功后 fire-and-forget 调用 `syncFavVideosToDb`，先查 sources 表获取 sourceId（收藏夹未 sync 则跳过）。per-video try/catch，DB 写入失败不影响 UI。MVP insert-only 不更新已有记录，`content_state='pending'`
