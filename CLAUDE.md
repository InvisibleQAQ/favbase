...
- `entrypoints/background.ts` — Background SW thin dispatcher：构建 BackgroundContext（tabAbortControllers + tabBvids + notifyTab + ensureOffscreen），`onMessage` switch 路由到独立 handler。~70 行
