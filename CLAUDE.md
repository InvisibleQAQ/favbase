...
- `docs/24_agent-bridge-reconnect-latency-remediation-2026-09-01.md` — Agent Bridge 重连延迟整改方案（Step 0 诊断 + Step 1-3 已落地 2026-09-01，Step 4-6 待实施）：核实「daemon 自灭 → 5 分钟退避」不成立（退避只由 `bad-token` 触发），定位四个真缺陷 D1 显式 connect-now 被退避门禁吞掉 / D2 daemon idle 计时不认扩展 peer / D3 alarm 周期与 helloWait 无契约（旧 Chrome 夹到 60s > 修复前 35s 必超时）/ D4 退避状态不衰减且不可观测；分 Step 0 诊断 → Step 1-4 止血 → Step 5 常连 → Step 6 Offscreen 迁移，Native Messaging/MCP/轮询已否决。**Step 0 诊断结论见 §9**：主动构造 bad-token 实证 §2 成立（token 不匹配 → 退避把「token 修好之后」继续锁住，实测 43s/3 次失败，曲线永不衰减且上限钉死 5 分钟）；冷启动基线 20s；本机 Chrome 149 不触发 D3 的旧版夹取；新增 F1 事故现场自我抹除（成功握手清零，D4 严重度上调）/ F2 daemon `reject()` 不写日志（Step 4-1 应改为 daemon 侧记录 reject reason + `/status` 透出，不动 v1 envelope）/ F3 `daemon.log` 无时间戳。**Step 1（修 D1）已落地 2026-09-01**：`AgentBridgeClient.tryConnect(trigger)` 单方法双触发（无平行 `forceConnect()`），`'user'` 跳过 `nextRetryAt` 门禁并在同一次 setStatus 里清零 `authFailureCount`/`nextRetryAt`（失败后回 30s 基数）；`scheduler.connectNow()` 是唯一 `'user'` 调用方，alarm/startup/config-watch/初始 refresh 均 `'schedule'`；顺手把 `handleRemoteClose` 并入 `disconnect()` 消除「不 close transport」的不对称（doc §8 第三条）。**Step 2（修 D2）已落地 2026-09-01**：`BridgeServer` 通过 `onPeerActivity` / `onPeerDisconnected` 报告已认证 hello、`pong`/`tools.result` 入站活动与 peer 断开，`Daemon.touch()` 在已认证 peer 存在时禁止 idle timer、断开后重新计时；§9.7 的 `createTransport` 同步抛出路径与 §8 的 connect-timeout 兜底按原文「无证据不动」保持未做。**Step 3（修 D3）已落地 2026-09-01**：保留 `AGENT_BRIDGE_POLL_MINUTES = 0.5`，将 daemon 默认 `helloWaitMs` 调整为 `75_000`（覆盖旧 Chrome 60s 实际 alarm 周期 + 15s 余量），两端常量注释声明跨包契约；测试继续注入 hello wait，未硬编码生产默认值。
