...
- `docs/26_platform-descriptor-consolidation-manual-2026-09-06.md` — **Platform Descriptor 收敛手册**（2026-09-06，三步分提交；**Step 1 已落地 2026-09-07**，Step 2-3 待执行）：把接入平台要手写的 16 处注册表收敛成 2 份 descriptor（`lib/collections/platform-descriptor.ts` 领域五字段 + `collection-platform-registry.ts` 的 `PLATFORM_META` 扩到 UI 五字段），并把 platform-onboarding.md §9 的无守卫清单从 **7 条压到 2 条**（复核时发现原 6 条外还有第 7 条：`capability-marquee.tsx` 的手工药丸表）。Step 1 零依赖前置**已落地**（`analytics-types.ts` 抽类型 / chat prompt 派生 / `.env.example` 转正 / marquee 覆盖断言 / 去掉 `overview-view.test.tsx` 四处硬编码 6）；**手册两处勘误已回写**——chat 平台字面量是 tools.ts **三处**（`:42` description 用显示名，手册漏记）且 `lib/chat/prompts.ts` 的 `CHAT_SYSTEM_PROMPT` 是手册范围外的第四处，**用户 2026-09-07 决定**四处全派生成 id 列表（显示名不可派生：`PLATFORM_META.title` 是 app 侧 `LocaleKeys`，`lib/` 引不到），并加 `lib/chat/tools.test.ts` 的「不得出示部分平台清单」守卫让 §9 第 3 条真死；**用户 2026-09-07 决定** `.env.example` 完整镜像 `.env.local` 结构（30 平台键 + LLM/ASR/Embedding/HTTP/Bridge 13 键，零密钥值，丢弃 `.env.local` 里零代码读取的 douyin 死块），守卫的 `.env.example` 半边改为**必须存在**；marquee 断言走 AST 复用已解析的 `PLATFORM_META`（手册说直接 import registry，改为零新增 import）→ Step 2 descriptor 落地 + 12 处改派生 + 删 `PLATFORM_LABEL` 与 `theme-config.platform` + 契约测试重写（**判据含 manifest 字节相同 diff**：MV3 已装扩展的 `host_permissions` 一变就要用户重新授权）→ Step 3 spec/CLAUDE.md 同步 + ADR 0004。四项经决策**排除**：4 处重值注册表不动、不修 `App.tsx` 的六平台 eager 图、marquee 不派生、不做 `platforms/<id>/` 物理 co-location。执行任一 Step 前先读该文对应节 + §1 决策记录 + §3 跨 Step 铁律
